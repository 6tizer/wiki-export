---
title: HEARTBEAT.md
type: concept
tags:
- OpenClaw
status: 审核中
confidence: high
last_compiled: '2026-05-01'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/03efd5b18d7c405e8bdb37ccc2367f31
notion_id: 03efd5b1-8d7c-405e-8bdb-37ccc2367f31
---

## 定义

[HEARTBEAT.md](http://heartbeat.md/) 是 OpenClaw 配置体系中的轻量巡检文件，用来定期检查上下文健康、未完成事项和记忆维护状态。

## 关键要点

- 目标是轻量巡检，而不是承担复杂决策

- 过长会放大上下文负担并削弱稳定性

- 与 [MEMORY.md](http://memory.md/) 等高频加载文件一样需要保持克制

## 来源引用

- [文章链接](https://x.com/0xKingsKuan/status/2032729079271817550)｜《OpenClaw 配置三原则：启动轻、分层严、记忆精》｜来源条目：OpenClaw 配置三原则：启动轻、分层严、记忆精

- 《clawchief：给 OpenClaw 装上“操作系统”，让 AI 真正替你跑一天》｜X书签文章｜原文链接：[https://x.com/AYi_AInotes/status/2040622308822483071](https://x.com/AYi_AInotes/status/2040622308822483071)｜来源条目：[摘要：clawchief：给 OpenClaw 装上“操作系统”，让 AI 真正替你跑一天](summaries/摘要：clawchief：给 OpenClaw 装上“操作系统”，让 AI 真正替你跑一天.md)

- 《OpenClaw 核心文档架构详尽分析与优化方案指南》｜X书签文章｜源文章：OpenClaw：用 Markdown 文件定义 AI Agent 大脑的「文件即代理」框架深度解析

## 关联概念

- [EVOLUTION.md](concepts/EVOLUTION.md.md)

- [受控自进化](concepts/受控自进化.md)

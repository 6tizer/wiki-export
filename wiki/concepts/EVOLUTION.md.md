---
title: EVOLUTION.md
type: concept
tags:
- OpenClaw
- Agent 编排
status: 草稿
confidence: high
last_compiled: '2026-04-16'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/32e7f812ced841089c6a0306e47b42b5
notion_id: 32e7f812-ced8-4108-9c6a-0306e47b42b5
---

## 定义

[EVOLUTION.md](http://evolution.md/) 是面向 Agent 自进化的独立沉淀文件，用来记录反思后的经验、启发式规则、失败教训与可复用最佳实践。

## 关键要点

- 它把“成长结果”从 [MEMORY.md](http://memory.md/) 的事实记忆中分离出来，单独管理可复用的方法论。

- 常与 [AGENTS.md](http://agents.md/) 中的 reflection loop 配合使用，形成“复盘—提炼—等待确认—应用”的闭环。

- 这种分层有助于把自进化限制在可审核边界内，避免直接改写 [SOUL.md](http://soul.md/) 带来的身份漂移。

## 来源引用

- 源文章：OpenClaw：用 Markdown 文件定义 AI Agent 大脑的「文件即代理」框架深度解析

- 源文章：Hermes Agent 核心文档优化指南：让你的 AI 助手真正「越用越聪明」｜来源条目：[摘要：Hermes 核心文档架构详尽分析与优化方案指南](summaries/摘要：Hermes 核心文档架构详尽分析与优化方案指南.md)

## 关联概念

- [AGENTS.md](concepts/AGENTS.md.md)

- [MEMORY.md](concepts/MEMORY.md.md)

- [HEARTBEAT.md](concepts/HEARTBEAT.md.md)

- [GEPA](concepts/GEPA.md)

- [Hermes Agent Self-Evolution](entities/Hermes Agent Self-Evolution.md)

- [USER.md](concepts/USER.md.md)

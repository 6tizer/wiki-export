---
title: team-agents.md
type: concept
tags:
- Agent 协作模式
- 多Agent协作
- 知识管理
status: 审核中
confidence: high
last_compiled: '2026-04-16'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/cd16acaacc4f42e0844c84ffb15523ae
notion_id: cd16acaa-cc4f-42e0-844c-84ffb15523ae
---

## 定义

[team-agents.md](http://team-agents.md/) 是多 Agent 团队的共享参考文件，用来记录团队 roster、何时调用哪个 profile、交接规则、权限上限与 cron 安排，帮助系统在数周后仍然保持边界清晰。

## 关键要点

- 它不是人格文件，而是团队运行说明书

- 适合沉淀角色职责、交接约定与审批边界

- 将 cron 时段统一写入同一文件，有助于提前发现调度冲突

- 放入版本控制后，可以追踪团队边界何时被扩大或改写

## 来源引用

- [原文链接](https://x.com/nyk_builderz/status/2044472463279710344)｜《The Ultimate Hermes Guide - from one agent to a 4-profile team that still feels coherent on day 30》｜源文章：Hermes 多 Agent 团队：如何让四个角色在第 30 天还保持分工清晰

## 关联概念

- [Hermes Agent](entities/Hermes Agent.md)

- [SOUL.md](concepts/SOUL.md.md)

- [AGENTS.md](concepts/AGENTS.md.md)

- [Handoff 文档](concepts/Handoff 文档.md)

---
title: Refusal Direction
type: concept
tags:
- LLM
- 安全/隐私
status: 审核中
confidence: high
last_compiled: '2026-04-12'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/4fac23e27a1c4decb973b9191b73d7a0
notion_id: 4fac23e2-7a1c-4dec-b973-b9191b73d7a0
---

## 定义

Refusal Direction 是对齐后模型在激活空间中形成的“拒答方向”，当输入触发安全边界时，模型会沿这一方向偏移并输出拒绝响应。

## 关键要点

- 它是理解模型为什么会拒答的一个表征视角

- Abliteration 一类方法的核心，就是识别并削弱这一方向

- 这个概念说明很多安全能力并不是离散规则，而是分布在表示空间中的统计结构

## 来源引用

- [原文链接](https://x.com/billtheinvestor/status/2040852920586129756)｜《Gemma-4-31B 被彻底解锁：Abliteration 技术如何从神经网络里切除安全护栏》｜源文章：Gemma-4-31B 被彻底解锁：Abliteration 技术如何从神经网络里切除安全护栏

- [原文链接](https://x.com/AYi_AInotes/status/2044964220005965927)｜《2026年的开源社区，已经不是人在玩AI了，已经进化到 AI在玩AI，玩得比人类还好很多😆😆😆》｜来源条目：[摘要：2026年的开源社区，已经不是人在玩AI了，已经进化到 AI在玩AI，玩得比人类还好很多😆😆😆](summaries/摘要：2026年的开源社区，已经不是人在玩AI了，已经进化到 AI在玩AI，玩得比人类还好很多😆😆😆.md)

## 关联概念

- [Gemma 4](entities/Gemma 4.md)

- [Abliteration](concepts/Abliteration.md)

- [过度对齐](concepts/过度对齐.md)

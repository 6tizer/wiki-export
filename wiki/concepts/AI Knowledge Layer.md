---
title: AI Knowledge Layer
type: concept
tags:
- 知识管理
- Agent 编排
status: 审核中
confidence: medium
last_compiled: '2026-04-15'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/6697536467ea483c99c258a596cb2f74
notion_id: 66975364-67ea-483c-99c2-58a596cb2f74
---

## 定义

AI Knowledge Layer 是位于人与 Agent 之间的一层知识基础设施，用来在执行任务前向 Agent 提供经过整理、可复用、可持续更新的上下文，而不是让模型每次都从零开始猜测。

## 关键要点

- 它把原始资料、笔记、书签、问答沉淀为结构化 Wiki，形成可查询的长期知识层。

- 它通常由动态的知识层与静态的风格/品牌层共同组成。

- 它强调“先编译、再使用”的知识组织方式，而不只是在查询时临时检索。

- 它适用于内容创作、团队协作和个人知识管理等多种场景。

## 来源引用

- [原文链接](https://x.com/shannholmberg/status/2044111115878326444)｜《AI Knowledge Layer (and why your agents are useless without it)》｜源文章：LLM Wikid：让你的 AI Agent 真正「认识你」的知识层架构

## 关联概念

- [Karpathy LLM Wiki 方法论](concepts/Karpathy LLM Wiki 方法论.md)

- [Context Engineering](concepts/Context Engineering.md)

- [Obsidian Web Clipper](entities/Obsidian Web Clipper.md)

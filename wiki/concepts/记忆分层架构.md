---
title: 记忆分层架构
type: concept
tags:
- 长期记忆
status: 审核中
confidence: high
last_compiled: '2026-04-28'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/a6ee038a74e3440fbf4ae114a23b6c55
notion_id: a6ee038a-74e3-440f-bf4a-e114a23b6c55
---

## 定义

记忆分层架构是一种 **Agent 长期记忆的组织模式**，通过将记忆按类型和用途分层、分目录管理，实现项目隔离和高效检索。Claude Code 的实现是该模式的典型案例。

## 核心要点

- **索引层 + 内容层**：[MEMORY.md](http://memory.md/) 作为顶层索引文件（新对话启动时自动读取前 200 行），下层按类别存放具体记忆

- **四类记忆分区**：

  - **user/**：用户个人信息和偏好

  - **feedback/**：用户对 Agent 的纠正或肯定

  - **project/**：项目进展、决策、背景

  - **reference/**：外部资源的指引

- **项目隔离**：每个项目拥有独立的记忆文件集，避免跨项目信息干扰

- **选择性加载**：不是一次性加载全部记忆，而是按需读取相关分区

## 设计启示

- 记忆质量比数量更重要：Opus 4.6 用 10 个精简文件远超 Sonnet 3.5 的 31 个杂乱文件

- 分层设计使记忆维护（Auto Dream）成为可能

## 相关概念

- Auto Dream

- Auto Memory

- Memory Folder

- [claude-obsidian](entities/claude-obsidian.md)

- [Claim 断言模型](concepts/Claim 断言模型.md)

- [llm-wiki](entities/llm-wiki.md)

- [Hermes Agent](entities/Hermes Agent.md)

- [Agent Skills](concepts/Agent Skills.md)

- [Cognee](entities/Cognee.md)

- [记忆整合](concepts/记忆整合.md)

- [图向量混合记忆](concepts/图向量混合记忆.md)

- [多跳关系检索](concepts/多跳关系检索.md)

- [Agent Harness](concepts/Agent Harness.md)

- [Agent 协议层](concepts/Agent 协议层.md)

- [Mediator 层](concepts/Mediator 层.md)

- [GenericAgent](entities/GenericAgent.md)

- [上下文信息密度最大化](concepts/上下文信息密度最大化.md)

- [自进化智能体](concepts/自进化智能体.md)

- [上下文压缩](concepts/上下文压缩.md)

- [外部化](concepts/外部化.md)

- [认知工件](concepts/认知工件.md)

## 来源引用

- [摘要：【万字】CC vs OpenClaw vs Hermes：一文深入拆解三大 Agent 框架](summaries/摘要：【万字】CC vs OpenClaw vs Hermes：一文深入拆解三大 Agent 框架.md)（[原文](https://mp.weixin.qq.com/s?__biz=MjM5NjE3NjYzMw%3D%3D&mid=2247496422&idx=1&sn=811535695480b3dedead76bdd49ebfc0&chksm=a707de0ce8531ae0d9d8a14dd5c1029fc10a9cf68cd3421d8de580646f70da173af4f671ec62#rd)）

- [摘要：Memory](summaries/摘要：Memory.md)（[原文](https://x.com/akshay_pachaar/status/2045510648474530263)）

- [摘要：Claude Code Auto Dream 记忆整理功能深度解析](summaries/摘要：Claude Code Auto Dream 记忆整理功能深度解析.md)— 数字生命卡兹克（微信公众号）

- [摘要：Claude Code Memory 记忆功能](summaries/摘要：Claude Code Memory 记忆功能.md)— 字节笔记本（微信，2026-02-27）：强调了层级化记忆管理方案，多层级规则和约定确保不同场景下灵活应用适当的记忆内容

- [摘要：龙虾养成日记PPT看不过瘾？内部版逐字稿来了](summaries/摘要：龙虾养成日记PPT看不过瘾？内部版逐字稿来了.md)（傅盛，2026-02-28）：傅盛的记忆系统经历五次进化（上下文 → 文件存储 → TODO 清单 → 向量化搜索），核心原则「不靠脑子，靠文件」

- [摘要：GitHub 上 OpenClaw 最实用的 20 个 Skills 和技巧](summaries/摘要：GitHub 上 OpenClaw 最实用的 20 个 Skills 和技巧.md)（牛皮糖不吹牛，2026-02-27）：cognitive-memory Skill 提供四种记忆类型（短期/长期/情景/语义）的多级记忆系统

- [摘要：Claude Code自动记忆来了！配合老金三层记忆系统全开源](summaries/摘要：Claude Code自动记忆来了！配合老金三层记忆系统全开源.md)（老金带你玩AI, 2026-02-28）— 三层 DIY 架构：Layer 1 Hooks 自动知识图谱（items.json + status 字段管理生命周期）、Layer 2 每日笔记、Layer 3 手动隐性知识。开源项目：[github.com/KimYx0207/claude-memory-3layer](http://github.com/KimYx0207/claude-memory-3layer)

- [摘要：我用OpenClaw搭了11个AI Agent，它们学会了自我进化](summaries/摘要：我用OpenClaw搭了11个AI Agent，它们学会了自我进化.md)（孟健AI编程，2026-03-01）— 两层记忆系统：长期记忆（[MEMORY.md](http://memory.md/)，架构决策/踩坑记录）+ 日记（memory/[YYYY-MM-DD.md](http://yyyy-mm-dd.md/)，每日操作/策略变更），核心原则「文件是唯一的真相来源」

- [摘要：OpenClaw 提速指南：一个加密交易员写出的 AI Agent 优化手册](summaries/摘要：OpenClaw 提速指南：一个加密交易员写出的 AI Agent 优化手册.md)（QingQ77，X书签，2026-03-17）— 用三级记忆与索引/检索/深度存储分层补充该架构的工程实践。

- [摘要：OpenClaw 提速指南：一个加密交易员写出的 AI Agent 优化手册](summaries/摘要：OpenClaw 提速指南：一个加密交易员写出的 AI Agent 优化手册.md)— X书签，2026-03-17：强调 [MEMORY.md](http://memory.md/) 只保留指针、详细内容下沉到 vault 的分层组织方式

- 《Claude + Obsidian | How to use your second brain》｜X书签文章｜原文链接：[https://x.com/defileo/status/2043762213597397179](https://x.com/defileo/status/2043762213597397179)｜来源条目：[摘要：Claude + Obsidian | How to use your second brain](summaries/摘要：Claude + Obsidian  How to use your second brain.md)

- [原文链接](https://mp.weixin.qq.com/s?__biz=MzkxNzY0OTA4Mg%3D%3D&mid=2247493206&idx=1&sn=7080809308368860641fd4df6601bef9&chksm=c0a8ea2ad24191e6e6e084a08eacab4b318f12dd484d75fe35bb554e96c6e9906e407e998911#rd)｜《Karpathy 提出了 LLM Wiki，我用 Rust 把它造出来了，还打通了 MemPalace 知识图谱》｜来源条目：[摘要：Karpathy 提出了 LLM Wiki，我用 Rust 把它造出来了，还打通了 MemPalace 知识图谱](summaries/摘要：Karpathy 提出了 LLM Wiki，我用 Rust 把它造出来了，还打通了 MemPalace 知识图谱.md)

- [原文链接](https://x.com/jiroucaigou/status/2044249069699428665)｜《Hermes Agent新手教程：从入门到精通，附带变现方式》｜来源条目：[摘要：Hermes Agent新手教程：从入门到精通，附带变现方式](summaries/摘要：Hermes Agent新手教程：从入门到精通，附带变现方式.md)

- [文章链接](https://x.com/akshay_pachaar/status/2043745099792953508)｜《Build Agents that never forget》｜来源页面：[摘要：Build Agents that never forget](summaries/摘要：Build Agents that never forget.md)

- [摘要：扣子2.5，开启全新 Agent World！](summaries/摘要：扣子2.5，开启全新 Agent World！.md)（[原文](https://mp.weixin.qq.com/s?__biz=Mzk0MzY0MTMwNA%3D%3D&mid=2247488354&idx=1&sn=4cf6c89cb341e49d85e218bfd9bf28ea&chksm=c2549c1180876b0ec346ce5adaf12e8f994675346315d6061cb27b6b06801e0d760224149c42#rd)）

- [摘要：首篇自进化智能体系统技术报告出炉：Token成本直降近10倍，省钱又高效！](summaries/摘要：首篇自进化智能体系统技术报告出炉：Token成本直降近10倍，省钱又高效！.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzA3MzI4MjgzMw%3D%3D&mid=2651029237&idx=1&sn=1c9efee6bf552718e0e795a795268f5a&chksm=852bf9401cd18c9b982dd29b8c4c78780b8965ee4b7bb27912e4fdb595df1baf0087f8ac6215#rd)）

- [摘要：上交大54页综述讲透Agent认知外部化的演进之路](summaries/摘要：上交大54页综述讲透Agent认知外部化的演进之路.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzA3MzI4MjgzMw%3D%3D&mid=2651029194&idx=2&sn=d468f16ac694682f072e150ac531e5a2&chksm=85cd02c33fc1ca1088db4f4f1a5b2219ed652157b32c004b7ef2cfb994654cfdfdd0d38512e7#rd)）

- [摘要：How to Give Claude Perfect Memory (complete guide)](summaries/摘要：How to Give Claude Perfect Memory (complete guide).md)（[原文](https://x.com/aiedge_/status/2046966170868486512)）

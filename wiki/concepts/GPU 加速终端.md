---
title: GPU 加速终端
type: concept
tags:
- 开发工具
status: 审核中
confidence: medium
last_compiled: '2026-04-11'
source_tags: 终端, 开发者工具, macOS
source_article_url: ''
notion_url: https://www.notion.so/0001fe40ae134bc1acf53bf2061b4bb8
notion_id: 0001fe40-ae13-4bc1-acf5-3bf2061b4bb8
---

## 定义

GPU 加速终端是通过图形处理器完成文本渲染、滚动与刷新的一类终端设计，用来提升长输出、高频交互和复杂界面场景下的流畅度。

## 关键要点

- 在长日志输出和高频交互下更容易体现优势

- 现代终端产品常把它作为性能卖点

- 适合 AI 编程工具的重度终端工作流

## 来源引用

- [原文链接](https://x.com/alin_zone/status/2033524177295274496)｜《Ghostty：HashiCorp 创始人打造的终端，49k Star 背后值得深扒》｜源文章：Ghostty：HashiCorp 创始人打造的终端，49k Star 背后值得深扒

## 关联概念

- [Ghostty](entities/Ghostty.md)

- [Quick Terminal](concepts/Quick Terminal.md)

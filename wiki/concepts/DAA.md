---
title: DAA
type: concept
tags:
- 商业/生态
status: 审核中
confidence: medium
last_compiled: '2026-04-27'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/f583f7c9cabf4d67ab8724b2838d4121
notion_id: f583f7c9-cabf-4d67-ab87-24b2838d4121
---

## 定义

DAA（Daily Active Agents）是用来衡量 AI 时代数字生产力的新指标，关注的是每天处于活跃状态、持续执行任务的智能体数量，而不只是人类用户的活跃度。

## 关键要点

- 它是对 DAU 的升级，更适合描述一个用户驱动多个 Agent 并行工作的现实

- DAA 的增长反映的是平台的智能体使用密度，而不只是人类访问量

- 在流量 2.0 语境下，DAA 往往比停留时长更能体现业务价值

- DAA 需要与任务完成率、词元消耗和智能体/用户比等指标一起理解

## 关联概念

- [流量 2.0](concepts/流量 2.0.md)

- [Token 经济学](concepts/Token 经济学.md)

- [AaaS 商业模式](concepts/AaaS 商业模式.md)

- [意图即交易](concepts/意图即交易.md)

## 来源引用

- [摘要：流量 2.0 时代](summaries/摘要：流量 2.0 时代.md)（[原文](https://mp.weixin.qq.com/s?__biz=MjM5OTE0ODA2MQ%3D%3D&mid=2650996683&idx=1&sn=aa6250a4cf1d43eda59489b2dc43e6e8&chksm=bd7fae4f5e696fc2d4d5ae5137085ac0911ce5f256732c241f544e82c9015d09658babf4a088#rd)）

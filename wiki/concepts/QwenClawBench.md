---
title: QwenClawBench
type: concept
tags: []
status: 审核中
confidence: medium
last_compiled: '2026-04-20'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/af37b9e7bbba4259aa71da95e0788824
notion_id: af37b9e7-bbba-4259-aa71-da95e0788824
---

## 定义

QwenClawBench 是用于评估模型在更贴近智能体编程任务中的表现的专项基准，侧重代码理解、任务拆解与工具协同能力。

## 关键要点

- 它服务于 Agentic Coding 场景，而不只是单轮代码生成。

- 文章把它列为 Qwen3.6-Max-Preview 表现领先的主要编程基准之一。

- 这类基准更强调模型是否能像开发者一样在复杂任务中持续推进。

## 关联概念

- [Qwen3.6-Max-Preview](entities/Qwen3.6-Max-Preview.md)

- [SWE-bench Pro](concepts/SWE-bench Pro.md)

- [Terminal-Bench 2.0](entities/Terminal-Bench 2.0.md)

## 来源引用

- [摘要：刚刚，千问最强模型发布，登顶国产最佳](summaries/摘要：刚刚，千问最强模型发布，登顶国产最佳.md)（[原文](https://mp.weixin.qq.com/s?__biz=MjM5MjAyNDUyMA%3D%3D&mid=2651088482&idx=1&sn=f82d4a5697aef837b57ea6353285d4be&chksm=bcff0435172cd02b80b80bc26991b8c0718cb409d76938cfb784f9aa171331118262579e5653#rd)）

---
title: ΛCDM 模型
type: concept
tags: []
status: 草稿
confidence: high
last_compiled: '2026-04-26'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/8958f087ff30459ea35a70de933e187e
notion_id: 8958f087-ff30-459e-a35a-70de933e187e
---

## 定义

ΛCDM 模型是现代宇宙学中的标准理论框架，其中 Λ 表示宇宙学常数、CDM 表示冷暗物质，用来解释宇宙的大尺度结构形成与演化。

## 关键要点

- 它以宇宙微波背景辐射等观测约束为基础，为数值模拟设定初始条件和整体演化框架。

- 在该模型下，宇宙结构通过引力作用由小到大逐步汇聚，形成星系、星系团与宇宙纤维网。

- “千衍”对 A2744 类似体的成功复现，被视为标准 ΛCDM 模型在极端并合环境下仍然有效的强证据。

## 来源引用

- [摘要：中国科学家用4.2万亿粒子构建虚拟宇宙，以“大衍之数”推演万物](summaries/摘要：中国科学家用4.2万亿粒子构建虚拟宇宙，以“大衍之数”推演万物.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzA3NTIyODUzNA%3D%3D&mid=2649795623&idx=1&sn=5c59215d07a7859ae045830e33f7179e&chksm=8634c6853fe8b94f48810fcb4a0f98abee3f451fa6f9fe0b8705f9798f52dd56d2ce68b975c0#rd)）

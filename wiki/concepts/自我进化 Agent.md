---
title: 自我进化 Agent
type: concept
tags:
- 记忆系统
- Agent 编排
status: 审核中
confidence: high
last_compiled: '2026-04-11'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/e81b87c95a69463d820d08aed46164c6
notion_id: e81b87c9-5a69-463d-820d-08aed46164c6
---

**定义**：自我进化 Agent 是一类能够从自身经验中自动学习、创造和改进技能的 AI 代理系统。它通过封闭学习循环实现自主优化，无需持续人工介入。

**关键特性**

- 从历史轨迹自动生成和改进技能

- 封闭学习循环：技能系统 + 持久化记忆 + 强化学习

- 支持批量轨迹生成和强化学习训练环境

- 能够深入理解用户并主动引用过去经验

**典型实现**：hermes-agent——开源的自我进化 AI 代理，支持批量轨迹生成、强化学习和持久化记忆

**来源引用**

- [摘要：AIGC开源推荐-自我进化学习-hermes-agent](summaries/摘要：AIGC开源推荐-自我进化学习-hermes-agent.md)

- [原文链接](https://x.com/berryxia/status/2034027088144155081)｜《MetaClaw：让你的 AI Agent 在每次对话后悄悄进化》｜来源条目：[摘要：MetaClaw：让你的 AI Agent 在每次对话后悄悄进化](summaries/摘要：MetaClaw：让你的 AI Agent 在每次对话后悄悄进化.md)

- [原文链接](https://mp.weixin.qq.com/s?__biz=MzI3MTA0MTk1MA%3D%3D&mid=2652692364&idx=1&sn=806ef08f63d5bf567d421f8808528c22&chksm=f047438bdb7b8b7734f13ec8b0c731b8c3a0085d501b9d6454c92b66b5d973bec5e7bce653ea#rd)｜《Hermes Agent抄袭中国团队代码实锤！被锤后回应：你删号》｜来源条目：[摘要：Hermes Agent抄袭中国团队代码实锤！被锤后回应：你删号](summaries/摘要：Hermes Agent抄袭中国团队代码实锤！被锤后回应：你删号.md)

## 关联概念

- [Hermes Agent](entities/Hermes Agent.md)

- [Hermes Agent Self-Evolution](entities/Hermes Agent Self-Evolution.md)

- [EvoMap](entities/EvoMap.md)

- [Evolver](entities/Evolver.md)

- [AI洗代码](concepts/AI洗代码.md)

- [持久化跨会话记忆](concepts/持久化跨会话记忆.md)

- [Claude Code](entities/Claude Code.md)

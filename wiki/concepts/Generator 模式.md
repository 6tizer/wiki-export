---
title: Generator 模式
type: concept
tags:
- 内容自动化
- AI 设计
- 提示工程
status: 审核中
confidence: high
last_compiled: '2026-04-26'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/8de6f5b5c14e4df49e290393927e3ee6
notion_id: 8de6f5b5-c14e-4df4-9e29-0393927e3ee6
---

## 定义

Generator 模式是通过模板和格式约束，让 Skill 专门负责稳定生成某一类结构化输出。

## 关键要点

- 适合 API 文档、测试用例、变更日志和报告等固定结构内容

- 重点是输出一致性，而不是让模型自由发挥

- 能把“写作风格”转化为可复用的生产规范

## 来源引用

- [摘要：Google 发布 5 个 Agent Skill 设计模式：告别乱写 SKILL.md 的时代](summaries/摘要：Google 发布 5 个 Agent Skill 设计模式：告别乱写 SKILL.md 的时代.md)（[原文链接](https://x.com/KKaWSB/status/2034196862794961299)）

- [原文链接](https://x.com/oragnes/status/2034221173970796776)｜《Google Cloud 的 5 种 [SKILL.md](http://skill.md/) 设计模式：让 AI 编程助手更听话的「抄作业」指南》

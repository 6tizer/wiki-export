---
title: SOUL.md
type: concept
tags:
- OpenClaw
status: 审核中
confidence: medium
last_compiled: '2026-04-23'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/455a7b48e4c54d1692be861c43062ea3
notion_id: 455a7b48-e4c5-4d16-92be-861c43062ea3
---

## 定义

[SOUL.md](http://soul.md/) 是 OpenClaw 的**核心个性化配置文件**，用户通过它定义 Agent 的个人偏好、背景信息、行为规范等，是 OpenClaw 九层 System Prompt 架构中用户层的核心载体。

## 作用

- 存储用户身份、偏好、工作习惯等信息

- 让 OpenClaw Agent 产生个性化响应

- 配置 Agent 的目标、约束和工作方式

## 来源引用

- [摘要：OpenClaw 配置三原则：启动轻、分层严、记忆精](summaries/摘要：OpenClaw 配置三原则：启动轻、分层严、记忆精.md)

- [摘要：Awesome OpenClaw Agents：187 个开箱即用的生产级 AI Agent 模板库](summaries/摘要：Awesome OpenClaw Agents：187 个开箱即用的生产级 AI Agent 模板库.md)

- [摘要：OpenClaw 这次最骚的更新，不是 Molty 更辣了，而是 Agent 开始有灵魂了](summaries/摘要：OpenClaw 这次最骚的更新，不是 Molty 更辣了，而是 Agent 开始有灵魂了.md)

## 来源引用

- [摘要：OpenClaw 配置三原则：启动轻、分层严、记忆精](summaries/摘要：OpenClaw 配置三原则：启动轻、分层严、记忆精.md)

- [摘要：Awesome OpenClaw Agents：187 个开箱即用的生产级 AI Agent 模板库](summaries/摘要：Awesome OpenClaw Agents：187 个开箱即用的生产级 AI Agent 模板库.md)

- [摘要：OpenClaw 这次最骚的更新，不是 Molty 更辣了，而是 Agent 开始有灵魂了](summaries/摘要：OpenClaw 这次最骚的更新，不是 Molty 更辣了，而是 Agent 开始有灵魂了.md)

- [摘要：OpenClaw 配置三原则：启动轻、分层严、记忆精](summaries/摘要：OpenClaw 配置三原则：启动轻、分层严、记忆精.md)

- [摘要：Awesome OpenClaw Agents：187 个开箱即用的生产级 AI Agent 模板库](summaries/摘要：Awesome OpenClaw Agents：187 个开箱即用的生产级 AI Agent 模板库.md)

- [摘要：OpenClaw 这次最骚的更新，不是 Molty 更辣了，而是 Agent 开始有灵魂了](summaries/摘要：OpenClaw 这次最骚的更新，不是 Molty 更辣了，而是 Agent 开始有灵魂了.md)

- [摘要：多Agent团队协作才是Hermes Agent的正确打开方式。](summaries/摘要：多Agent团队协作才是Hermes Agent的正确打开方式。.md)

- [摘要：Hermes Agent 从中级到高级进阶指南](summaries/摘要：Hermes Agent 从中级到高级进阶指南.md)

- [摘要：抽丝剥茧：深度解析 Hermes Agent 万字系统提示词（System Prompt）构成](summaries/摘要：抽丝剥茧：深度解析 Hermes Agent 万字系统提示词（System Prompt）构成.md)

- [摘要：The Ultimate Hermes Guide - from one agent to a 4-profile team that still feels coherent on day 30](summaries/摘要：The Ultimate Hermes Guide - from one agent to a 4-profile team that still feels coherent on day 30.md)

- [摘要：Hermes Agent 高阶工具全配置：从身份记忆到感知表达，一文打通](summaries/摘要：Hermes Agent 高阶工具全配置：从身份记忆到感知表达，一文打通.md)（[原文](https://x.com/ResearchWang/status/2045812932538438001)）

- 《OpenClaw 核心文档架构详尽分析与优化方案指南》｜X书签文章｜源文章：OpenClaw：用 Markdown 文件定义 AI Agent 大脑的「文件即代理」框架深度解析

- 源文章：Hermes Agent 核心文档优化指南：让你的 AI 助手真正「越用越聪明」｜来源条目：[摘要：Hermes 核心文档架构详尽分析与优化方案指南](summaries/摘要：Hermes 核心文档架构详尽分析与优化方案指南.md)

- [摘要：终于有了一个用龙虾而不是 Claude Code 的理由](summaries/摘要：终于有了一个用龙虾而不是 Claude Code 的理由.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzA4NzgzMjA4MQ%3D%3D&mid=2453482949&idx=1&sn=509fac8677449b751dadbb35cb9ec661&chksm=865bf05dbe70dd46c861dfc7a98cfe9b0625c81b849121816b05ca23a2cc8e15976c17ef9009#rd)）

- [摘要：The @karpathy SOUL is now live:](summaries/摘要：The @karpathy SOUL is now live-.md)（[原文](https://x.com/aeonframework/status/2046964241211507055)）

## 关联概念

- [Hermes Agent](entities/Hermes Agent.md)

- [多 Agent 协作工作流](concepts/多 Agent 协作工作流.md)

- [AGENTS.md](concepts/AGENTS.md.md)

- [OpenClaw 九层 System Prompt 架构](concepts/OpenClaw 九层 System Prompt 架构.md)

- [内容工厂工作流](concepts/内容工厂工作流.md)

- [IDENTITY.md](concepts/IDENTITY.md.md)

- [MEMORY.md](concepts/MEMORY.md.md)

- [Andrej Karpathy](entities/Andrej Karpathy.md)

- [OpenClaw bootstrap 分层](concepts/OpenClaw bootstrap 分层.md)

- [Molty SOUL](concepts/Molty SOUL.md)

- [Agent 人格层](concepts/Agent 人格层.md)

- [Push back 协作](concepts/Push back 协作.md)

- [Agent-curated memory](concepts/Agent-curated memory.md)

- [ModelBox](entities/ModelBox.md)

- [Memory KPI](concepts/Memory KPI.md)

- [Policy Gate](concepts/Policy Gate.md)

- [team-agents.md](concepts/team-agents.md.md)

- [STYLE.md](concepts/STYLE.md.md)

- [文件即大脑](concepts/文件即大脑.md)

- [受控自进化](concepts/受控自进化.md)

- [USER.md](concepts/USER.md.md)

- [agency-agents-zh](entities/agency-agents-zh.md)

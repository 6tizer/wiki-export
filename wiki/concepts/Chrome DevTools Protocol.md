---
title: Chrome DevTools Protocol
type: concept
tags:
- 浏览器自动化
status: 审核中
confidence: medium
last_compiled: '2026-04-26'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/8333a0550eea4916a9165ab3ab0778d8
notion_id: 8333a055-0eea-4916-a916-5ab3ab0778d8
---

## 定义

Chrome DevTools Protocol 是 Chrome 暴露给外部程序的调试与控制协议，可被自动化脚本用来操控页面、DOM、网络和浏览器行为。

## 关键要点

- 是许多浏览器自动化工具的底层接口

- 能实现真实浏览器操作、调试与状态读取

- 常被用于 Agent 驱动的网页执行场景

## 来源引用

- [摘要：xiaohongshu-skills：让 AI Agent 真正「操控」小红书的 CDP 自动化方案](summaries/摘要：xiaohongshu-skills：让 AI Agent 真正「操控」小红书的 CDP 自动化方案.md)（[原文](https://x.com/QingQ77/status/2031197851443769531)）

- [摘要：OpenClaw 养虾踩坑实录：如何用 CDP 把浏览器完全交给 AI Agent 控制](summaries/摘要：OpenClaw 养虾踩坑实录：如何用 CDP 把浏览器完全交给 AI Agent 控制.md)（[原文](https://x.com/billtheinvestor/status/2031202833987702875)）

- [原文链接](https://x.com/TokenMore/status/2032806514692599813)｜《chrome-cdp-skill：让 AI Agent 直连你正在用的 Chrome，无需重新登录》｜来源条目：[摘要：chrome-cdp-skill：让 AI Agent 直连你正在用的 Chrome，无需重新登录](summaries/摘要：chrome-cdp-skill：让 AI Agent 直连你正在用的 Chrome，无需重新登录.md)

- [原文链接](https://x.com/yan5xu/status/2032858943874281782)｜《bb-browser：用你真实的 Chrome 登录态，把整个互联网变成 AI Agent 的 CLI》｜来源条目：[摘要：bb-browser：用你真实的 Chrome 登录态，把整个互联网变成 AI Agent 的 CLI](summaries/摘要：bb-browser：用你真实的 Chrome 登录态，把整个互联网变成 AI Agent 的 CLI.md)

- [摘要：这个 Skill 太硬了，刚开源就斩获 2.8K 星标！Agent 联网能力拉满！](summaries/摘要：这个 Skill 太硬了，刚开源就斩获 2.8K 星标！Agent 联网能力拉满！.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzIxNzQwNjM3NA%3D%3D&mid=2247546207&idx=1&sn=772820dba9a7a0a037b4bad24eee51ad&chksm=962fbc784ab760b642a982a17ee1b7a251fc8dd1573eef97759423d6400035677c10ff9ab5ae#rd)）

- [原文链接](https://x.com/milesdeutscher/status/2044536031991763414)｜《How to Connect Claude to TradingView (FULL GUIDE)》｜来源条目：[摘要：How to Connect Claude to TradingView (FULL GUIDE)](summaries/摘要：How to Connect Claude to TradingView (FULL GUIDE).md)

- [原文链接](https://mp.weixin.qq.com/s/2k5S5UlZKqACLr8g1Mb2eQ)｜《真香！这可能是openclaw抓取任何网页的终极解决方案了》｜源文章：真香！这可能是openclaw抓取任何网页的终极解决方案了

- [摘要：self-healing](summaries/摘要：self-healing.md)（[原文](https://x.com/Gorden_Sun/status/2046228429662794153)）

- [摘要：agent-browser：为AI Agent 写的浏览器CLI](summaries/摘要：agent-browser：为AI Agent 写的浏览器CLI.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzIwNzU5MzY4OQ%3D%3D&mid=2247486082&idx=1&sn=68860cb626f874e6753331df42a06452&chksm=96b464c960e585a7580e8b47dab3d5957471c51e820dc9014909b4afcba510e60109f7838c82#rd)）

## 关联概念

- [chrome-cdp-skill](entities/chrome-cdp-skill.md)

- [web-access](entities/web-access.md)

- [CDP Proxy](concepts/CDP Proxy.md)

- [Chrome DevTools MCP](entities/Chrome DevTools MCP.md)

- [Dokobot](entities/Dokobot.md)

- [Browser Harness](entities/Browser Harness.md)

- [agent-browser](entities/agent-browser.md)

- [可访问性树](concepts/可访问性树.md)

- [Snapshot 引用机制](concepts/Snapshot 引用机制.md)

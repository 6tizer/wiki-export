---
title: GDPVal
type: concept
tags:
- 商业/生态
- 模型评测
status: 审核中
confidence: medium
last_compiled: '2026-04-28'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/a1f0dad8862d487d8bc883831c0e3d9d
notion_id: a1f0dad8-862d-487d-8bc8-83831c0e3d9d
---

## 定义

GDPVal 是一个用于评估 AI 实际工作能力的 GDP 验证数据集，包含 220 个基于真实经济场景的职业任务，覆盖 44 个行业领域。

## 核心要点

- 220 个真实职业任务，涵盖制造业、金融、医疗、法律等 44 个行业

- 报酬按美国劳工统计局（BLS）时薪标准计算（$82.78-$5004/任务）

- 平均每个任务价值 $259.45

- 任务需要交付真实文档（Word、Excel、PDF）

- 用 GPT-5.2 按特定领域评分标准进行质量评分

## 来源引用

- [摘要：ClawWork AI 经济压力测试框架](summaries/摘要：ClawWork AI 经济压力测试框架.md)（开源星探，2026-02-18）

- [摘要：小米 MiMo-V2.5 系列开源 & Orbit 百万亿 Token 计划启动](summaries/摘要：小米 MiMo-V2.5 系列开源 & Orbit 百万亿 Token 计划启动.md)（[原文](https://mp.weixin.qq.com/s?__biz=Mzk3NTkxMTM2NA%3D%3D&mid=2247484681&idx=1&sn=60241f47e569b8105e66bf704cd4c98b&chksm=c50137dcd08f21a4eb67a631268f2b59dcc62a811ba5f31be372f09d38e5300b8474690cc111#rd)）

---
title: IDENTITY.md
type: concept
tags:
- Agent 协作模式
- 提示工程
- OpenClaw
status: 审核中
confidence: high
last_compiled: '2026-04-11'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/486c74e49de542f38999b734601a83ec
notion_id: 486c74e4-9de5-42f3-8999-b734601a83ec
---

## 定义

[IDENTITY.md](http://identity.md/) 是 OpenClaw 工作区中的核心配置文件之一，用来定义 Agent 的身份、人设、行为边界和长期偏好。

## 关键要点

- 它属于用户可直接编辑的高层提示入口

- 与 [GUIDELINES.md](http://guidelines.md/)、[SOUL.md](http://soul.md/) 等文件一起构成个性化行为约束

- 通过文件化配置，用户可以持续迭代 Agent 的长期行为而不依赖一次性对话注入

## 来源引用

- 《OpenClaw 的 9 层 System Prompt 架构：框架管稳定，你管个性化》

- 《把 Agent 当新员工带：OpenClaw 深度使用心得》（@sitinme，X书签）— 从新员工培养视角补充 [IDENTITY.md](http://identity.md/) 在身份设定中的实践意义

- [原文链接](https://x.com/0xKingsKuan/status/2032729079271817550)｜《OpenClaw 配置三原则：启动轻、分层严、记忆精》｜来源条目：[摘要：OpenClaw 配置三原则：启动轻、分层严、记忆精](summaries/摘要：OpenClaw 配置三原则：启动轻、分层严、记忆精.md)

- 《OpenClaw 核心文档架构详尽分析与优化方案指南》｜X书签文章｜源文章：OpenClaw：用 Markdown 文件定义 AI Agent 大脑的「文件即代理」框架深度解析

## 关联概念

- [SOUL.md](concepts/SOUL.md.md)

- [MEMORY.md](concepts/MEMORY.md.md)

- [OpenClaw bootstrap 分层](concepts/OpenClaw bootstrap 分层.md)

- [STYLE.md](concepts/STYLE.md.md)

- [TOOLS.md](concepts/TOOLS.md.md)

---
title: Memory Lock-in
type: concept
tags:
- 长期记忆
- Harness 工程
- 知识管理
status: 审核中
confidence: high
last_compiled: '2026-04-13'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/4387a46ddd3a4cc9b7f95ba753804c50
notion_id: 4387a46d-dd3a-4cc9-b7f9-5ba753804c50
---

## 定义

Memory Lock-in 指 Agent 的历史状态、用户偏好、长期记忆与压缩产物被绑定到特定平台或模型提供商，导致一旦迁移 Harness、模型或部署平台，就无法无损继承既有经验的锁定效应。

## 关键要点

- 真正难迁移的往往不是模型本身，而是已经积累下来的状态与记忆

- Stateful API、server-side compaction 与黑箱长期记忆都会放大这种锁定

- 一旦记忆与 Harness 被整体收进托管 API，开发者会同时失去可见性、可控性与可迁移性

## 关联概念

- [agentmemory](entities/agentmemory.md)

- [持久化跨会话记忆](concepts/持久化跨会话记忆.md)

- [Agent Harness](concepts/Agent Harness.md)

- [Deep Agents](entities/Deep Agents.md)

- [Claude Managed Agents](entities/Claude Managed Agents.md)

- [Compaction](concepts/Compaction.md)

- [Open Memory](concepts/Open Memory.md)

## 来源引用

- [摘要：local-first AI assistant built for personal AI sovereignty](summaries/摘要：local-first AI assistant built for personal AI sovereignty.md)（[原文](https://x.com/ghumare64/status/2045291112454402194)）

- [原文链接](https://x.com/hwchase17/status/2042978500567609738)｜《Your harness, your memory》｜源文章：Harrison Chase：你的 Agent Harness，就是你的记忆

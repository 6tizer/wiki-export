---
title: Vibe Coding
type: concept
tags:
- AI 设计
- 代码生成
- Agent 协作模式
status: 已审核
confidence: high
last_compiled: '2026-05-03'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/cf5d62b518344617b2248e1fd8765f8b
notion_id: cf5d62b5-1834-4617-b224-8e1fd8765f8b
---

## 定义

Vibe Coding（氛围编程）是一种以 AI 为执行者、人为决策者的协作编程方法论，通过规范化的五阶段工作流实现高效的人机协作开发。

## 关键要点

- **五阶段循环**：需求描述 → 架构设计 → 代码生成 → 调试优化 → 持续迭代

- 需求描述分三步：模糊需求澄清 → 技术选型 → 结构化确认

- 代码生成逐模块进行，每次给 AI 精准的上下文

- 三轮规则：同一问题超过 3 轮未解决，立刻开新对话

- 阶段产物（需求文档、架构图、调试日志）是资产，下次迭代直接复用

- 核心原则：AI 是执行者，你是决策者

- 在 Coze 2.5 场景中，Vibe Coding 被进一步产品化为手机对话式生成网页、小程序与工作流的交互方式

## 来源引用

- [摘要：别问 Claude 能为你做什么，问你能为 Claude 做什么：Anthropic 内部的 Vibe Coding 方法论](summaries/摘要：别问 Claude 能为你做什么，问你能为 Claude 做什么：Anthropic 内部的 Vibe Coding 方法论.md)（[原文](https://x.com/GoSailGlobal/status/2046785975582667071)）

- [摘要：Vibecoding Sucks & Long Running Frameworks are Boring](summaries/摘要：Vibecoding Sucks & Long Running Frameworks are Boring.md)（[原文](https://x.com/gsd_foundation/status/2045188911807295489)）

- [摘要：我给了他一个梦想：超越 Claude Code](summaries/摘要：我给了他一个梦想：超越 Claude Code.md)（[原文](https://mp.weixin.qq.com/s?__biz=MjM5NDk5MTA0MQ%3D%3D&mid=2652331368&idx=1&sn=0f4117cecb5e356189ebcacdbf50c07e&chksm=bc5a95231d52bd399537c084f44729c05c12299bad4f0ca346266958f977576301a54030911d#rd)）

- [摘要：Vibe Coding规范工作流](summaries/摘要：Vibe Coding规范工作流.md)（清新研究/清华大学沈阳教授团队，2026-02-27）

- [摘要：AI开发范式——Spec Kit、OpenSpec、BMAD 全解析](summaries/摘要：AI开发范式——Spec Kit、OpenSpec、BMAD 全解析.md)（娄晨，2026-03-03）— 分析 Vibe Coding 在大型项目中的三大问题（上下文丢失、文档-代码脱节、技术债加速）及人的判断力退化风险

- [摘要：搞钱副业：AI小程序从0到上架](summaries/摘要：搞钱副业：AI小程序从0到上架.md)（@ai_xiaomu，X书签，2026-04-09）— 介绍零基础用户通过 Vibe Coding 方式（截图 + 对话循环）在 1-2 天内完成微信小程序 MVP 的实战案例

- [摘要：本周 GitHub 增长最快的十个 AI 仓库：Vibe Coding 的「道法术器」哲学凭什么拿下 10 万星？](summaries/摘要：本周 GitHub 增长最快的十个 AI 仓库：Vibe Coding 的「道法术器」哲学凭什么拿下 10 万星？.md)

- [摘要：一个人，三个月，十几个项目：idoubicc 的 Vibe Coding 实验全记录](summaries/摘要：一个人，三个月，十几个项目：idoubicc 的 Vibe Coding 实验全记录.md)

- 本养虾人看哭了！字节扣子2.5出生即满级，手机对话就能Vibe Coding（微信）

- [摘要：扣子编程 CLI，对所有 Agent 全面开放](summaries/摘要：扣子编程 CLI，对所有 Agent 全面开放.md)

- [摘要：Claude Code 更新又遭泄露，Cursor 们的好日子到头了](summaries/摘要：Claude Code 更新又遭泄露，Cursor 们的好日子到头了.md)

- [摘要：阿里下场做了中国版的 Lovable，好用到炸。](summaries/摘要：阿里下场做了中国版的 Lovable，好用到炸。.md)

- [摘要：就一个 skills，凭啥 4w Star？](summaries/摘要：就一个 skills，凭啥 4w Star？.md)

- [摘要：我用1分钟开发了个上线应用，有阿里Meoo谁还学编程啊](summaries/摘要：我用1分钟开发了个上线应用，有阿里Meoo谁还学编程啊.md)（微信）

- [摘要：一个冷门无人提的冷知识却又是程序员常识：](summaries/摘要：一个冷门无人提的冷知识却又是程序员常识：.md)

- [摘要：从Vibe Coding到Wish Coding，AI编程迎来C端拐点](summaries/摘要：从Vibe Coding到Wish Coding，AI编程迎来C端拐点.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzA3MzI4MjgzMw%3D%3D&mid=2651028663&idx=1&sn=d0213992629afda755d05c81935f071c&chksm=85b8bbe09ea5a23dbe4d0d34e05714297cd747857268248dc27f65dce8caf03822cfe3e62cfb#rd)）

- [摘要：做了十年设计，这次真觉得自己多余了｜GPT-Image-2 上手实测](summaries/摘要：做了十年设计，这次真觉得自己多余了｜GPT-Image-2 上手实测.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzU0MDk3NTUxMA%3D%3D&mid=2247496592&idx=1&sn=ff0599ff60bf5fbbd07b5b18d626c9cd&chksm=faeab77a93137cfd2ee02971d6731162f1562e3b988e6d38a012b94116546d5ade80a17278c6#rd)）

- [摘要：Karpathy AI Ascent 2026：从 Vibe Coding 到 Agentic Engineering](summaries/摘要：Karpathy AI Ascent 2026：从 Vibe Coding 到 Agentic Engineering.md)（[原文](https://x.com/Av1dlive/status/2049561210593685876)）

- [摘要：软件 3.0 时代来临](summaries/摘要：软件 3.0 时代来临.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzA4NzgzMjA4MQ%3D%3D&mid=2453483261&idx=1&sn=cb6b022f6372657475687143a4ab33bb&chksm=860f87620bc9685632e714cdaddbe215724ff7e2328e46057f2f9ff940644865d447f06ac7e5#rd)）

- [摘要：Karpathy AI Ascent 2026 三大新视野](summaries/摘要：Karpathy AI Ascent 2026 三大新视野.md)（[原文](https://x.com/karpathy/status/2049903821095354523)）

- [摘要：Claude Code + Gemma 4 一键理清Mac 30000+图片](summaries/摘要：Claude Code + Gemma 4 一键理清Mac 30000+图片.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzIzMzQyMzUzNw%3D%3D&mid=2247516043&idx=1&sn=e2b7667a501ddf0fae30c4e3e5d34253&chksm=e970810309a2dcc99d38de4807aa58df859fda450a4a518557848e649aacceb7f9364127a2ac#rd)）

- [摘要：我用 AI 做了个修仙游戏两天，从视频号梗到可下载试玩](summaries/摘要：我用 AI 做了个修仙游戏两天，从视频号梗到可下载试玩.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzkzODk0NjA3NA%3D%3D&mid=2247484459&idx=1&sn=04ceddcb719602758aa35b18e5613d20&chksm=c334d8db51774650be7aff2d7a2b2b408d63ceba43850f1d74e137c012e7c88653c4542c63d8#rd)）

- [摘要：Build: Karpathy 2026 AI Agent Playbook](summaries/摘要：Build- Karpathy 2026 AI Agent Playbook.md)（[原文](https://x.com/DataChaz/status/2050114234973863975)）

## 关联概念

- [日抛软件](concepts/日抛软件.md)

- [Claude Code](entities/Claude Code.md)

- [yoyo](entities/yoyo.md)

- [Harness Engineering](concepts/Harness Engineering.md)

- 自进化 Agent

- [Claude Managed Agents](entities/Claude Managed Agents.md)

- [Cursor](entities/Cursor.md)

- [模型即应用](concepts/模型即应用.md)

- [模型中立](concepts/模型中立.md)

- [Meoo](entities/Meoo.md)

- [蜂群模式](concepts/蜂群模式.md)

- [RLS 行级安全策略](concepts/RLS 行级安全策略.md)

- [可追溯日志体系](concepts/可追溯日志体系.md)

- [Wish Coding](concepts/Wish Coding.md)

- [Zero DevOps](concepts/Zero DevOps.md)

- [结构化意图表示层](concepts/结构化意图表示层.md)

- [消费级 Coding Agent](concepts/消费级 Coding Agent.md)

- [蚂蚁灵光](entities/蚂蚁灵光.md)

- [闪应用](entities/闪应用.md)

- [Vibe Proofing](concepts/Vibe Proofing.md)

- [Spike 实验](concepts/Spike 实验.md)

- [Spec-driven 开发](concepts/Spec-driven 开发.md)

- [GPT-Image-2](entities/GPT-Image-2.md)

- [Seedance 2.0](entities/Seedance 2.0.md)

- [叶子节点策略](concepts/叶子节点策略.md)

- [可验证性设计](concepts/可验证性设计.md)

- [Agentic Engineering](concepts/Agentic Engineering.md)

- [Software 3.0](concepts/Software 3.0.md)

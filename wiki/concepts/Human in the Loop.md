---
title: Human in the Loop
type: concept
tags:
- 工作流
- Agent 编排
status: 草稿
confidence: high
last_compiled: '2026-04-15'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/a27eacc4aef048f6bb62b69f8f80a7f0
notion_id: a27eacc4-aef0-48f6-bb62-b69f8f80a7f0
---

## 定义

**Human in the Loop** 是一种将人工审核嵌入自动化流程的控制机制。系统先自动生成候选方案或评分结果，再由人类在关键节点确认、否决或校正。

别名：人在回路

## 关键要点

- 适合“机器可做初筛，但结果仍需人为判断”的任务

- 常用于质量把关、风险控制、主观评审和高影响决策

- 与全自动流程的差别不在于效率，而在于把不确定性留给人来兜底

- 在达尔文.skill 中，它体现在每轮优化后暂停，展示 diff、分数变化和测试输出，等待人工确认

## 来源引用

- 《达尔文.skill正式发布，一个无限进化的skill系统！》｜X书签文章｜原文链接：[https://x.com/AlchainHust/status/2043709317296361851](https://x.com/AlchainHust/status/2043709317296361851)｜来源条目：达尔文.skill：用进化论的棘轮机制，让你的 Claude Code Skill 自动迭代升级

- 《Github 上的硅基生物 —— 一手达尔文、一手女娲 ，先造人后进化》｜微信文章｜原文链接：[https://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247493022&idx=1&sn=624a417e7c20389f39d7c12d68db105c&chksm=c03f662e4b855b2d7f8982c96394e2f3f90f128f81904761993d26762b66605dac072cafc190#rd](https://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ%3D%3D&mid=2247493022&idx=1&sn=624a417e7c20389f39d7c12d68db105c&chksm=c03f662e4b855b2d7f8982c96394e2f3f90f128f81904761993d26762b66605dac072cafc190#rd)｜源文章：Github 上的硅基生物 —— 一手达尔文、一手女娲 ，先造人后进化

## 关联概念

- [达尔文.skill](entities/达尔文.skill.md)

- [autoresearch](entities/autoresearch.md)

- [棘轮机制](concepts/棘轮机制.md)

---
title: Long-Horizon Coding
type: concept
tags:
- Coding Agent
- 工作流
- 记忆系统
- 上下文管理
- 长期记忆
status: 审核中
confidence: high
last_compiled: '2026-04-22'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/8ae84932b2aa4d7b908567731ce1b05a
notion_id: 8ae84932-b2aa-4d7b-9085-67731ce1b05a
---

## 定义

Long-Horizon Coding 是指 AI 在长时间、多步骤、多文件、多工具调用的连续软件工程任务中保持目标一致性、上下文稳定性与执行闭环的能力。

## 关键要点

- 它关注的不是单轮代码生成，而是持续数小时的任务推进、重试、修复与跨文件协同。

- 文章将 4,000+ 次工具调用与 12 小时连续执行作为这一能力的直观指标。

- 对 Coding Agent 而言，Long-Horizon Coding 是从“会写代码”走向“能完成工程”的关键分水岭。

## 来源引用

- [摘要：Ship Safe v9.1.0](summaries/摘要：Ship Safe v9.1.0.md)（[原文](https://x.com/Kimi_Moonshot/status/2046249571882500354)）

- [摘要：ziggy-llm](summaries/摘要：ziggy-llm.md)（[原文](https://x.com/zxytim/status/2046255419178500408)）

## 关联概念

- [Kimi K2.6](entities/Kimi K2.6.md)

- [Kimi Code](entities/Kimi Code.md)

- [Agent Swarms](concepts/Agent Swarms.md)

---
title: 链上交易 Skill
type: concept
tags:
- 加密资产
- 链上协议
- 量化交易
status: 审核中
confidence: medium
last_compiled: '2026-05-01'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/57a8778e5af84effa5fe26cdd5caf063
notion_id: 57a8778e-5af8-4eff-a5fe-26cdd5caf063
---

## 定义

链上交易 Skill 是把行情筛选、合约查询、风险检查和交易执行封装成可调用技能的 Agent 能力单元。

## 关键要点

- 把扫链、调研和下单流程标准化成可复用操作

- 适合高频、低时延和步骤固定的交易场景

- 常通过 API Key 直连真实钱包或交易平台

## 来源引用

- [摘要：XXYY Trade Skill：5分钟给你的 AI Agent 装上链上交易大脑](summaries/摘要：XXYY Trade Skill：5分钟给你的 AI Agent 装上链上交易大脑.md)｜X书签文章｜原文链接：[https://x.com/PepeBoost888/status/2032052111010382031](https://x.com/PepeBoost888/status/2032052111010382031)

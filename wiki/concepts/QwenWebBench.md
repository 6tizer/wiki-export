---
title: QwenWebBench
type: concept
tags:
- 模型评测
- 浏览器自动化
status: 审核中
confidence: medium
last_compiled: '2026-04-27'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/67b589c676c54942b1d8f3b5b029c712
notion_id: 67b589c6-76c5-4942-b1d8-f3b5b029c712
---

## 定义

QwenWebBench 是面向网页任务与浏览器交互能力的评测基准，用于观察模型在检索、页面理解与网页操作相关任务中的完成度。

## 关键要点

- 它反映模型是否能把网页作为工作环境，而不只是回答网页内容相关问题。

- 文章将其纳入 Qwen3.6-Max-Preview 编程能力提升的证据链之一。

- 这类基准通常与工具调用、页面状态理解和多步骤执行能力相关。

## 关联概念

- [Qwen3.6-Max-Preview](entities/Qwen3.6-Max-Preview.md)

- [Terminal-Bench 2.0](entities/Terminal-Bench 2.0.md)

## 来源引用

- [摘要：刚刚，千问最强模型发布，登顶国产最佳](summaries/摘要：刚刚，千问最强模型发布，登顶国产最佳.md)（[原文](https://mp.weixin.qq.com/s?__biz=MjM5MjAyNDUyMA%3D%3D&mid=2651088482&idx=1&sn=f82d4a5697aef837b57ea6353285d4be&chksm=bcff0435172cd02b80b80bc26991b8c0718cb409d76938cfb784f9aa171331118262579e5653#rd)）

---
title: Vaults
type: concept
tags: []
status: 审核中
confidence: high
last_compiled: '2026-04-13'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/de0ba365ac704f5ca7421859e5e9b132
notion_id: de0ba365-ac70-4f5c-a742-1859e5e9b132
---

## 定义

Vaults 是 Anthropic Managed Agents 体系中的凭证管理机制，用于隔离和管理用户访问外部服务所需的密钥。

## 关键要点

- 文章强调 Vaults 的核心价值在于把外部服务密钥与模型上下文隔离

- 它体现了托管式 Agent 平台把权限、凭证与执行环境一体化封装的思路

- 对多外部服务 Agent 工作流来说，Vaults 是平台级安全能力的一部分

## 来源引用

- [原文链接](https://mp.weixin.qq.com/s?__biz=Mzg3MTk3NzYzNw%3D%3D&mid=2247506328&idx=1&sn=d03e32eb701b71176450f7b8bda36481&chksm=cf8ba6a23e6803733918f630b2e14ee243187d68061fbad56d96e4c3b04b6a39ba50ef7611a8#rd)｜《Mythos造假/Opus降智/Agent新平台，Anthropic所有更新一次性看懂》｜源文章：Mythos造假/Opus降智/Agent新平台，Anthropic所有更新一次性看懂

---
title: Verse
type: concept
tags:
- 开发工具
- Coding Agent
status: 审核中
confidence: medium
last_compiled: '2026-04-23'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/e44d9e146a5e476fba40f4f4da1e0078
notion_id: e44d9e14-6a5e-476f-ba40-f4f4da1e0078
---

## 定义

Verse 是 Epic 面向 Fortnite / UEFN 生态推出的编程语言，用来为地图、玩法和交互逻辑编写可执行规则。

## 关键要点

- 在这篇材料里，Verse 被视为过去阻挡普通创作者进入 Fortnite 开发的重要门槛之一。

- 作者的核心判断是：当 Claude 已能根据自然语言描述生成 Verse 代码时，非专业开发者也能更快把玩法想法落地成地图原型。

- 但回复区也提醒，Verse 代码只是地图开发的一部分，复杂项目仍需要玩法设计、资产制作、测试和分发能力。

## 来源引用

- [摘要：20-year-old Chinese developer made more than $100,000 starting by building products for a kids game, he simply transferred the object tracking model thanks to Verse.](summaries/摘要：20-year-old Chinese developer made more than $100,000 starting by building products for a kids game, he simply transferred the object tracking model thanks to Verse.md)（[原文](https://x.com/noisyb0y1/status/2046962651373109474)）

---
title: Brand Foundation
type: concept
tags: []
status: 审核中
confidence: medium
last_compiled: '2026-04-15'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/fbec5059c8004b25afcc452689d0d14a
notion_id: fbec5059-c800-4b25-afcc-452689d0d14a
---

## 定义

Brand Foundation 是 AI Knowledge Layer 中保持静态的人类控制层，用来沉淀品牌定位、语气规则、视觉偏好、禁用表达和受众定义，确保 Agent 输出始终贴近既定风格。

## 关键要点

- 这一层应由人维护，不应被 Agent 自动改写。

- 它关注“如何说”和“像谁说”，而不是“知道什么”。

- 与动态的 Knowledge Base Layer 配合后，可以同时提升内容准确性与风格一致性。

- 对个人品牌、内容团队和营销型 Agent 特别重要。

## 来源引用

- [原文链接](https://x.com/shannholmberg/status/2044111115878326444)｜《AI Knowledge Layer (and why your agents are useless without it)》｜源文章：LLM Wikid：让你的 AI Agent 真正「认识你」的知识层架构

## 关联概念

- [Context Engineering](concepts/Context Engineering.md)

- [Karpathy LLM Wiki 方法论](concepts/Karpathy LLM Wiki 方法论.md)

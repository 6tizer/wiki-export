---
title: Knowledge MEMO
type: concept
tags:
- 知识管理
- 工作流
status: 草稿
confidence: high
last_compiled: '2026-04-20'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/871423f942f74a26b572f15991a323fd
notion_id: 871423f9-42f7-4a26-b572-f15991a323fd
---

## 定义

Knowledge MEMO 是本文作者提出的一套面向 AI 时代的个人知识系统方法，强调知识库不是单纯给 Agent 调用的上下文仓库，而是帮助人类完成理解、判断、复习与长期积累的脚手架。

## 关键要点

- 它把知识管理视为“工具箱 + 纪律”而不是一次性配置好的产品。

- 系统强调人在 loop，拒绝零摩擦自动入库，要求读、记、确认都由人主动参与。

- 相比通用模板，它更鼓励从个人真实兴趣出发，逐步长成自己的知识地图。

- 它在 Ingest、Query、Lint 之外补上 Retain，要求把知识真正推进长期记忆。

## 来源引用

- [摘要：Karpathy或许答错了一个根本问题：wiki 是为谁准备的？](summaries/摘要：Karpathy或许答错了一个根本问题：wiki 是为谁准备的？.md)（[原文](https://mp.weixin.qq.com/s?__biz=Mzg3MTEwMzQ2OA%3D%3D&mid=2247483810&idx=1&sn=fcd68f9a5f2134ca0abad5f744566a7c&chksm=cf7a6df266c8aeb90040a7ce83e0446ecb6187f3ae9527d78573b6803138c64a677ca46eabcc#rd)）

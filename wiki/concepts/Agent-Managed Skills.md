---
title: Agent-Managed Skills
type: concept
tags:
- Harness 工程
status: 审核中
confidence: medium
last_compiled: '2026-04-15'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/c3c6630891e34e4fbdf26cdda6d268fd
notion_id: c3c66308-91e3-4e4f-bdf2-6cdda6d268fd
---

## 定义

Agent-Managed Skills 是 Hermes 把重复成功的工作流沉淀为可复用 Skill 的机制。它让 Agent 不只“记住经验”，还能把经验整理成触发条件、操作步骤、注意事项和验证方法。

## 关键要点

- 它对应的是程序性知识，而不只是事实记忆

- 目标是把非平凡工作流固化为可复用 SOP，减少重复推理成本

- 高质量 Skill 通常需要清晰的触发条件、可执行步骤、注意事项和验证方法

- 当某类经验反复出现在记忆里时，更适合升级为 Skill，而不是继续散落在记忆文件中

## 来源引用

- [原文链接](https://x.com/BTCqzy1/status/2044259795499450414)｜《Hermes Agent 从中级到高级进阶指南》｜源文章页面：Hermes Agent 进阶指南：记忆系统、技能自进化与多 Agent 协作全解析

- [原文链接](https://x.com/NeoAIForecast/status/2044252861710905685)｜《How Skills Work in Hermes Agent》｜来源条目：[摘要：How Skills Work in Hermes Agent](summaries/摘要：How Skills Work in Hermes Agent.md)

## 关联概念

- [Hermes Agent](entities/Hermes Agent.md)

- [AGENTS.md](concepts/AGENTS.md.md)

- [Agent Skills](concepts/Agent Skills.md)

- [程序性记忆](concepts/程序性记忆.md)

- [渐进式披露](concepts/渐进式披露.md)

- [SKILL.md](concepts/SKILL.md.md)

---
title: Agent 动力学
type: concept
tags:
- Agent 协作模式
status: 审核中
confidence: medium
last_compiled: '2026-04-26'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/d0624ffe29d042c8898431a765de1d1e
notion_id: d0624ffe-29d0-42c8-8984-31a765de1d1e
---

## 定义

Agent 动力学是用于理解多 Agent 在共享环境中如何分工、协作、竞争、形成群体印象与组织文化的一种分析视角。

## 关键要点

- 关注 agent 数量、角色分工、共享记忆、Token 成本与结果质量之间的非线性关系

- 认为多 Agent 组织会逐渐形成类似企业文化的群体记忆和行为模式

- 用户的管理方式会影响组织形态：协作式会促进共创，竞争式会诱发赛马与“办公室政治”

- 这不是纯技术问题，也与管理学、组织学、社会学密切相关

- 可用于指导 agent-native team 的规模、分工与治理设计

## 关联概念

- [Multi-Agent 群聊](concepts/Multi-Agent 群聊.md)

- [任务认领机制](concepts/任务认领机制.md)

- [外部记忆](concepts/外部记忆.md)

- [Slock](entities/Slock.md)

## 来源引用

- [摘要：用 Agent 动力学，和 40 个 Agents 一起为「人 + AI」做产品｜42章经](summaries/摘要：用 Agent 动力学，和 40 个 Agents 一起为「人 + AI」做产品｜42章经.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzIyMDE5OTYyMw%3D%3D&mid=2651051564&idx=1&sn=babc1b1e75b77826a63d304d33c348e0&chksm=8dd911d417729de737132383124e9f75bf193b1c2ce831b08c3e001f0552438ebac7eae45e1d#rd)）

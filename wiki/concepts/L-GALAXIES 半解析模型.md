---
title: L-GALAXIES 半解析模型
type: concept
tags: []
status: 草稿
confidence: high
last_compiled: '2026-04-26'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/7b5656db97ab40a5bbe0cd3479c25058
notion_id: 7b5656db-97ab-40a5-bbe0-cd3479c25058
---

## 定义

L-GALAXIES 半解析模型是一类基于暗物质结构演化结果，为模拟结果补充星系形成与可观测属性的半解析建模方法。

## 关键要点

- 纯暗物质 N 体模拟只能给出引力骨架，无法直接生成星系颜色、光度等重子物理相关量。

- 在“千衍”研究里，团队借助 L-GALAXIES 为筛选出的暗物质类似体填入星系物理属性，并进一步生成接近 JWST 视角的虚拟观测。

- 它连接了大尺度暗物质模拟与面向观测的星系性质预测，是理论模拟走向可观测对比的重要桥梁。

## 来源引用

- [摘要：中国科学家用4.2万亿粒子构建虚拟宇宙，以“大衍之数”推演万物](summaries/摘要：中国科学家用4.2万亿粒子构建虚拟宇宙，以“大衍之数”推演万物.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzA3NTIyODUzNA%3D%3D&mid=2649795623&idx=1&sn=5c59215d07a7859ae045830e33f7179e&chksm=8634c6853fe8b94f48810fcb4a0f98abee3f451fa6f9fe0b8705f9798f52dd56d2ce68b975c0#rd)）

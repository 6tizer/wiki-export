---
title: Molty SOUL
type: concept
tags:
- OpenClaw
- Agent 编排
status: 审核中
confidence: medium
last_compiled: '2026-04-26'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/690db72bb4d2437faf5c467e4508b5d2
notion_id: 690db72b-b4d2-437f-af5c-467e4508b5d2
---

## 定义

Molty SOUL 是 OpenClaw 社区传播开的一套 **Agent 人格改写方案**，核心做法不是额外叠一层花哨提示词，而是直接重写 [SOUL.md](concepts/SOUL.md.md)，把 Agent 的语气、判断力与协作风格显式配置出来。

## 关键要点

- 它强调 **强观点、少废话、敢判断**，反对企业客服式的安全话术

- 它把人格视为可运营的系统层，而不是一次性 prompt 小技巧

- 它保留业务边界、安全规则与审批纪律，只重点改造“语气层”和“协作气质”

- 它的价值不只是更有趣，而是让 Agent 更适合长期协作

## 来源引用

- [摘要：OpenClaw 这次最骚的更新，不是 Molty 更辣了，而是 Agent 开始有灵魂了](summaries/摘要：OpenClaw 这次最骚的更新，不是 Molty 更辣了，而是 Agent 开始有灵魂了.md)（[原文链接](https://mp.weixin.qq.com/s?__biz=MzI0Mjc0MzA0Mg%3D%3D&mid=2247485445&idx=1&sn=76cc8a9f0947218acfbaeb3abaaf10cf&chksm=e87d5f08412f5b5a3cd9c4e9c75c6ba3c77347420e6699e8e54fe3e6be0b703447bf8d9c0065#rd)）

## 关联概念

- [OpenClaw](entities/OpenClaw.md)

- [SOUL.md](concepts/SOUL.md.md)

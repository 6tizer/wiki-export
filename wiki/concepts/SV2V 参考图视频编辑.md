---
title: SV2V 参考图视频编辑
type: concept
tags:
- 视频/3D
- 多模态
status: 草稿
confidence: medium
last_compiled: '2026-04-27'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/c23e4477d40342229501c1a1dce2ae36
notion_id: c23e4477-d403-4222-9501-c1a1dce2ae36
---

## 定义

SV2V（Style Video-to-Video）参考图视频编辑是一种通过上传参考图片来驱动视频风格或主体替换的技术。用户仅需提供一张目标风格或角色的参考图像，AI 即可将视频中的主体或整体风格无缝替换为参考图所指定的效果。

## 关键要点

- 支持视频主体替换（如将视频中的小狗替换为小猫）

- 支持整体风格迁移（如将实拍视频转为吉卜力动画风格）

- 与 V2V 自然语言编辑互补，分别对应图像驱动和文本驱动两种编辑范式

- HappyHorse-1.0 同时支持 V2V 和 SV2V 两种编辑模式

## 关联概念

- [HappyHorse](entities/HappyHorse.md)

- [V2V 自然语言视频编辑](concepts/V2V 自然语言视频编辑.md)

## 来源引用

- [摘要：阿里「快乐小马」来了，首批网友已玩疯！720P低至0.44元/秒](summaries/摘要：阿里「快乐小马」来了，首批网友已玩疯！720P低至0.44元-秒.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzI3MTA0MTk1MA%3D%3D&mid=2652695709&idx=1&sn=28912efd11e0430f1f71113f20567298&chksm=f037f03efe37824790e1a6bac199399193ffb1f824b4c24ca3fd750435e3491eab69b8ee94d5#rd)）

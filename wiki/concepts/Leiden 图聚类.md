---
title: Leiden 图聚类
type: concept
tags:
- 知识管理
status: 审核中
confidence: high
last_compiled: '2026-04-20'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/e660f4ac2a8a496eaef23726f256ffd4
notion_id: e660f4ac-2a8a-496e-aef2-3726f256ffd4
---

## 定义

Leiden 图聚类是一种基于图拓扑结构进行社区发现的方法，可把高连接度的节点群自动划分为相对紧密的语义社区。

## 关键要点

- 它不依赖向量 Embedding，也能通过边的连接密度识别概念之间的内在聚合关系。

- 在知识图谱场景中，Leiden 图聚类可直接把显式关系网络转化为可浏览的主题社区。

- 相比黑盒相似度检索，这种方法更便于解释“为什么这些概念会被归为一组”。

## 来源引用

- [摘要：Graphify-让Karpathy方法构建的知识库实现71.5倍效率提升](summaries/摘要：Graphify-让Karpathy方法构建的知识库实现71.5倍效率提升.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzcwMjIwMDk2Mg%3D%3D&mid=2247483836&idx=1&sn=87efe38320d976ed8850cded7a9919e8&chksm=f5738db7a5afe6ffd094c3453e8877b6266c1beda961858dc1d414bc80f787a3f74103f66e4c#rd)）

## 关联概念

- [Graphify](entities/Graphify.md)

- [Karpathy LLM Wiki 方法论](concepts/Karpathy LLM Wiki 方法论.md)

- [AST+LLM 双阶段提取](concepts/AST+LLM 双阶段提取.md)

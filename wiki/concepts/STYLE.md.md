---
title: STYLE.md
type: concept
tags:
- OpenClaw
- Agent 编排
status: 审核中
confidence: high
last_compiled: '2026-04-23'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/bba51b1e51554f2388b0cc3af9a71be5
notion_id: bba51b1e-5155-4f23-88b0-cc3af9a71be5
---

## 定义

[STYLE.md](http://style.md/) 是 OpenClaw 社区常见的风格分层文件，用来把 Agent 的“怎么说话”从 [SOUL.md](http://soul.md/) 中拆分出来，单独承载语气、格式、表达边界与输出示例。

## 关键要点

- 它把人格与表达解耦，降低 [SOUL.md](http://soul.md/) 过长和身份漂移的风险。

- 适合沉淀 do's & don'ts、响应结构模板、格式约束等高频风格规则。

- 在生产稳定版中，[STYLE.md](http://style.md/) 与 [SOUL.md](http://soul.md/)、[AGENTS.md](http://agents.md/) 形成“价值观—表达—执行”的三层分工。

## 来源引用

- 源文章：OpenClaw：用 Markdown 文件定义 AI Agent 大脑的「文件即代理」框架深度解析

- [摘要：The @karpathy SOUL is now live:](summaries/摘要：The @karpathy SOUL is now live-.md)（[原文](https://x.com/aeonframework/status/2046964241211507055)）

## 关联概念

- [SOUL.md](concepts/SOUL.md.md)

- [AGENTS.md](concepts/AGENTS.md.md)

---
title: Claude Code Routines
type: concept
tags: []
status: 审核中
confidence: high
last_compiled: '2026-04-20'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/85f5b637d4cf4e669b83ae64fa107705
notion_id: 85f5b637-d4cf-4e66-9b83-ae64fa107705
---

## 定义

Claude Code Routines 是 Claude Code Desktop 提供的**云端自动化任务能力**，可以把 Prompt、仓库访问权限和 Connectors 打包成可重复运行的任务，在 Anthropic 的云端按计划或事件触发执行。

## 关键要点

- 不依赖本地电脑持续在线，适合替代必须常驻本机的 Scheduled Tasks

- 支持三类触发方式：定时调度、GitHub 事件触发、API 调用触发

- 面向开发团队的常见自动化场景，例如 Issue triage、PR review digest、依赖更新检查、Flaky test 跟踪

- 本质上是把 Coding Agent 从“交互式编程助手”扩展为“可托管运行的自动化执行器”

## 来源引用

- [摘要：ThinkInAI Weekly AI周刊 VOL.42 Opus 4.7 来了、Qwen 开源 35B、Agent 工具链全面升级](summaries/摘要：ThinkInAI Weekly AI周刊 VOL.42 Opus 4.7 来了、Qwen 开源 35B、Agent 工具链全面升级.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzA4ODg0NDkzOA%3D%3D&mid=2247514009&idx=1&sn=bf15a5a46c8fceb7d84d0b3760d4ade9&chksm=91d6d8d58aad639487948529ced9d124bf6b7e77e01022df6a25570b93843ec362146982cbe9#rd)）

- [原文链接](https://mp.weixin.qq.com/s?__biz=MzIzMzQyMzUzNw%3D%3D&mid=2247515443&idx=1&sn=83324258b9f56b464f858f0deb0962a1&chksm=e99cecb1a90cbe4df1285e9b663144d03d9aa6cf67951e02148b275d61446970490fc395284c#rd)｜《全新重构版Claude Code Desktop上线！把Codex APP抄了个遍》｜原始条目：全新重构版Claude Code Desktop上线！把Codex APP抄了个遍

- [原文链接](https://x.com/claudeai/status/2044095086460309790)｜《Now in research preview: routines in Claude Code.》｜来源条目：[摘要：Now in research preview: routines in Claude Code.](summaries/摘要：Now in research preview- routines in Claude Code.md)

- [原文链接](https://x.com/claudeai/status/2044131493966909862)｜《Learn more in the official documentation》｜来源条目：[摘要：Learn more in the official documentation](summaries/摘要：Learn more in the official documentation.md)

## 关联概念

- [Claude Code](entities/Claude Code.md)

- [Git Worktree](concepts/Git Worktree.md)

- [托管 Agent](concepts/托管 Agent.md)

- [Claude Agent SDK](entities/Claude Agent SDK.md)

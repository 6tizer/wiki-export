---
title: System Prompt 工程
type: concept
tags: []
status: 审核中
confidence: high
last_compiled: '2026-04-23'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/8d99067df76d4bcb8981b702fe0e6ea7
notion_id: 8d99067d-f76d-4bcb-8981-b702fe0e6ea7
---

## 定义

System Prompt 工程是把模型的角色设定、行为边界、输出格式与问题求解流程系统化设计的实践，用来稳定提升 Agent 在真实任务中的表现。

## 关键要点

- 它不只是“写一段更长的提示词”，而是设计一套可迁移、可维护的行为约束系统

- 在多模型切换、子 Agent 调度与复杂任务执行中，常常比换模型本身更重要

- 与上下文注入、角色定义、任务验证机制结合后，能够明显拉齐不同模型的执行质量

## 来源引用

- [摘要：What is an Agent Harness](summaries/摘要：What is an Agent Harness.md)（[原文](https://x.com/aparnadhinak/status/2046980769747533830)）

- [原文链接](https://x.com/outsource_/status/2040345259335422463)｜《模型只是引擎，System Prompt 才是底盘：用 Claude 优化自身的迁移实验》｜源文章：模型只是引擎，System Prompt 才是底盘：用 Claude 优化自身的迁移实验

- [原文链接](https://x.com/KKaWSB/status/2043119101028274268)｜《Evolutionary self-improvement for Hermes Agent》｜来源条目：[摘要：Evolutionary self-improvement for Hermes Agent](summaries/摘要：Evolutionary self-improvement for Hermes Agent.md)

- [原文链接](https://x.com/LawrenceW_Zen/status/2043580374501249175)｜《While 循环谁都会写，上下文工程才是真功夫》｜来源条目：[摘要：While 循环谁都会写，上下文工程才是真功夫](summaries/摘要：While 循环谁都会写，上下文工程才是真功夫.md)

## 关联概念

- [Hermes Agent Self-Evolution](entities/Hermes Agent Self-Evolution.md)

- [Hermes Agent](entities/Hermes Agent.md)

- [GEPA](concepts/GEPA.md)

- [DSPy](entities/DSPy.md)

- [技能自我进化](concepts/技能自我进化.md)

- [Context Engineering](concepts/Context Engineering.md)

- [Agent Loop](concepts/Agent Loop.md)

- [工具描述](concepts/工具描述.md)

- [上下文窗口](concepts/上下文窗口.md)

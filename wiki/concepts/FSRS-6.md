---
title: FSRS-6
type: concept
tags:
- 知识管理
- 记忆系统
status: 草稿
confidence: medium
last_compiled: '2026-04-20'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/a02e086ddb7e4ecdafa1a8c9528204b1
notion_id: a02e086d-db7e-4ecd-afa1-a8c9528204b1
---

## 定义

FSRS-6 是一种用于间隔重复调度的算法模型，文中被用作 /review 环节的技术基础，用来决定一张知识卡片应该在什么时间再次复习。

## 关键要点

- 它将“复习哪张卡、何时复习”从主观感觉变成可计算的节奏。

- 在本文语境里，FSRS-6 是 Retain 落地为日常习惯的关键基础设施。

- 算法只能负责排程，真正坐下来复习的人仍然必须是使用者自己。

- 它体现了作者对“知识进脑”而非“仅可搜索”的偏好。

## 来源引用

- [摘要：Karpathy或许答错了一个根本问题：wiki 是为谁准备的？](summaries/摘要：Karpathy或许答错了一个根本问题：wiki 是为谁准备的？.md)（[原文](https://mp.weixin.qq.com/s?__biz=Mzg3MTEwMzQ2OA%3D%3D&mid=2247483810&idx=1&sn=fcd68f9a5f2134ca0abad5f744566a7c&chksm=cf7a6df266c8aeb90040a7ce83e0446ecb6187f3ae9527d78573b6803138c64a677ca46eabcc#rd)）

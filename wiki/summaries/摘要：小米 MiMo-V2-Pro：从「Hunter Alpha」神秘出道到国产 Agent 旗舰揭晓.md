---
title: 摘要：小米 MiMo-V2-Pro：从「Hunter Alpha」神秘出道到国产 Agent 旗舰揭晓
type: summary
tags:
- Agent 协作模式
- 上下文管理
status: 已审核
confidence: medium
last_compiled: '2026-04-11'
source_tags: LLM, Agent
source_article_url: https://www.notion.so/335701074b6881338ff9e3252d21f9f4
notion_url: https://www.notion.so/Tizer/930eb8eaad8540efb11c8a1734bac91e
notion_id: 930eb8ea-ad85-40ef-b11c-8a1734bac91e
---

### 一句话摘要

MiMo-V2-Pro 通过匿名盲测与 1M 上下文、Agent 优化能力的组合，把自己定位成国产模型里面向复杂工作流的一张新牌。

### 关键洞察

- 它通过 OpenRouter 匿名测试先建立市场关注，再完成品牌揭晓。

- 1M 上下文与 Agent 场景优化，是它区别于同类国产模型的核心卖点。

- 适合长代码库分析、复杂工作流和大上下文任务，但并非全能替代方案。

- 社区反馈普遍认为它在 Agent 场景有竞争力，但稳定性与幻觉率仍要继续观察。

### 提取的概念

- [MiMo-V2-Pro](entities/MiMo-V2-Pro.md)

- [1M 上下文](concepts/1M 上下文.md)

- [Agent Team](concepts/Agent Team.md)

### 原始文章信息

- 作者：Khazix0918

- 来源：X书签

- 发布时间：未明确披露

- 链接：[原文链接](https://x.com/Khazix0918/status/2034306301841596614)

### 个人评注

这类条目对 Tizer 的价值，在于帮助区分“适合沉淀为概念的方法”与“需要跟踪的模型实体”。MiMo-V2-Pro 更偏后者，适合持续观察其在 Agent 场景里的真实口碑。 

---
title: 摘要：Nunchi agent-cli：在 Hyperliquid 上跑 14 个开源量化策略，还能接 Claude Code 和 MCP
type: summary
tags:
- 量化交易
- MCP 协议
- CLI 工具
- OpenClaw
status: 已审核
confidence: high
last_compiled: '2026-04-11'
source_tags: OpenClaw, Agent, 自动化, LLM
source_article_url: https://www.notion.so/335701074b688173ae8fd0ec5533d44d
notion_url: https://www.notion.so/Tizer/35ac4a6b80834d2f8ec434d628e92088
notion_id: 35ac4a6b-8083-4d2f-8ec4-34d628e92088
---

## 一句话摘要

Nunchi agent-cli 把 Hyperliquid 上的多种量化策略、风险控制和 AI 调用接口打包成一个可直接被 Agent 使用的交易工具栈。

## 关键洞察

- 它的亮点不只是 14 个开源策略，而是把选币、执行、止损和复盘都模块化了。

- APEX、Radar、Guard、REFLECT 共同构成了一个带反馈闭环的自主交易系统。

- 通过 MCP、CLI 和 OpenClaw Skill 接口，交易框架开始直接面向 Agent 暴露能力。

- 公开策略并不等于可直接盈利，真实风险仍在滑点、手续费和极端行情里。

## 提取的概念

- [Nunchi](entities/Nunchi.md)

- [Hyperliquid](entities/Hyperliquid.md)

- [APEX 多槽位编排器](concepts/APEX 多槽位编排器.md)

- [Radar 机会雷达](concepts/Radar 机会雷达.md)

- [Guard 动态止损](concepts/Guard 动态止损.md)

- [REFLECT 夜间自我复盘](concepts/REFLECT 夜间自我复盘.md)

## 原始文章信息

- 作者：xincctnnq（币圈女菩萨｜Pizza披萨🍕）

- 来源：X书签

- 发布时间：2026-03-14

- 链接：[原文](https://x.com/xincctnnq/status/2032772661722624497)

## 个人评注

这类项目对 Tizer 的启发在于，垂直行业 Agent 不是只会“聊天”，而是开始直接接入真实执行系统。只是越接近真钱，HITL 和风控越不能省。

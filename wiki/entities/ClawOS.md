---
title: ClawOS
type: entity
tags:
- 量化交易
- AI 产品
- 链上协议
status: 审核中
confidence: medium
last_compiled: '2026-04-17'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/fd9cc7d88e8c41a2b20cdf91676b4249
notion_id: fd9cc7d8-8e8c-41a2-b20c-df91676b4249
---

## 定义

ClawOS 是一个在浏览器中运行的自托管私有云桌面系统，将 AI 炒股分析、个人应用与远程访问能力整合到统一 Web 桌面中。

## 关键要点

- 提供接近原生桌面的交互体验，包括 Dock、窗口管理、通知中心和桌面组件

- 将 AI 股票分析作为核心亮点，结合多专家投票、止损纪律、风控与专家记忆系统

- 支持本地部署和通过 VPN 远程访问，大多数外部服务均为可选项

## 来源引用

- [原文链接](https://mp.weixin.qq.com/s/5hQGUC6o-vLdcHVxVsOQwg)｜《Hermes Agent 网页控制台； AI原生社交平台；自主机器学习研究平台》｜源文章：Hermes Agent 网页控制台； AI原生社交平台；自主机器学习研究平台

---
title: Google ADK
type: entity
tags:
- Agent 框架
- 记忆系统
status: 草稿
confidence: medium
last_compiled: '2026-04-11'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/c2c71b473615428da275461f0a844871
notion_id: c2c71b47-3615-428d-a275-461f0a844871
---

## 定义

Google ADK 是 Google 提供的 Agent Development Kit，用于构建、编排和运行 Agent 应用。

## 关键要点

- 提供 Agent 运行与工具调用的开发底座

- 在 Always-On Memory Agent 中承担系统框架角色

- 适合需要接入 Google 模型与 Agent 示例工程的开发者

## 关联概念

- [Agent Harness](concepts/Agent Harness.md)

- [Antigravity](entities/Antigravity.md)

- [Gemini CLI](entities/Gemini CLI.md)

- [A2A 协议](concepts/A2A 协议.md)

## 来源引用

- [原文链接](https://x.com/berryxia/status/2031406673148166584)｜《Always-On Memory Agent：Google 开源的「会思考的大脑」，让你的 AI 助手不再失忆》｜来源条目：Always-On Memory Agent：Google 开源的「会思考的大脑」，让你的 AI 助手不再失忆

- [原文链接](https://mp.weixin.qq.com/s?__biz=MzkyNjU2ODM2NQ%3D%3D&mid=2247627539&idx=1&sn=da04448ed2172c5381d68b24e0454247&chksm=c31182e7c7ef6bde7a36b46d0bcde4b0b20f049b7cdbbe53d6ae77d1d83ea99c2250b5f96f56#rd)｜《别人都在卷Harness， 而Google 的沉默振聋发聩》｜源文章：别人都在卷Harness， 而Google 的沉默振聋发聩

---
title: recursive-mode
type: entity
tags:
- 工作流
- 记忆系统
status: 审核中
confidence: medium
last_compiled: '2026-04-26'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/0f1a0d90afa74fa09eb73c33f53c8936
notion_id: 0f1a0d90-afa7-4fa0-9eb7-3c33f53c8936
---

## 定义

recursive-mode 是一个免费开源工具，主张把需求、状态、决策与结构化记忆沉淀到代码仓库文件中，以对抗长会话中的上下文腐烂。

## 关键要点

- 用仓库内文档代替易丢失的聊天上下文

- 通过 `.recursive/` 目录组织记忆、状态与决策记录

- 强调跨会话、跨贡献者、跨仓库的持续可追溯性

- 代表一种“记忆属于代码仓库，而不属于某个 Agent 或 Harness”的思路

## 关联概念

- [Harness Engineering](concepts/Harness Engineering.md)

- [Ralph Loop](concepts/Ralph Loop.md)

- [PRD 驱动 Vibe Coding](concepts/PRD 驱动 Vibe Coding.md)

## 来源引用

- [原文链接](https://mp.weixin.qq.com/s?__biz=MzA5MTIxNTY4MQ%3D%3D&mid=2461159297&idx=1&sn=e1d1550d8c29f6c57f344e9bd7619931&chksm=8645d4fd11f0bef28663c05083551df88d222c61fd3c8390df4b43f46267e00d6b3a54bcfe38#rd)｜《Harrison Chase：你的 Agent Harness 就是你的记忆，一旦选了闭源，数据就不是你的了》｜源文章：Harrison Chase：你的 Agent Harness 就是你的记忆，一旦选了闭源，数据就不是你的了

- [原文链接](https://x.com/wayne_zhang0/status/2042874483606983079)｜[摘要：Ralph Loop：更轻量的 Harness Engineering 循环框架](summaries/摘要：Ralph Loop：更轻量的 Harness Engineering 循环框架.md)｜X书签文章

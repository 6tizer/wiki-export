---
title: EvoMap
type: entity
tags:
- 商业/生态
- Agent 协作模式
status: 审核中
confidence: medium
last_compiled: '2026-04-26'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/ffef30ea9995461c8296a75b305e6094
notion_id: ffef30ea-9995-461c-8296-a75b305e6094
---

## 定义

EvoMap 是一个围绕 AI Agent 自进化能力构建的中国团队与生态，核心代表是自进化引擎 **Evolver**，强调让 Agent 像“基因系统”一样继承、变异和持续优化能力。

## 关键要点

- 核心方向是让 Agent 在真实运行中持续积累经验，并把能力升级沉淀为可复用机制。

- 文中给出的生态数据包括：13 万个 Agent 节点接入、累计资产超 138 万、总调用超 4600 万次。

- 文中称 Evolver 在 ClawHub 上线 10 分钟登顶热门榜首，前三天下载量突破 36000+。

- 这篇文章将 EvoMap 描述为 Hermes Agent 架构相似性争议中的原始提出方，并公开发布了技术对比材料。

- 在争议之后，团队决定将 Evolver 核心模块改为混淆发布，并把协议从 MIT 调整为 GPL-3.0。

## 来源引用

- [原文链接](https://mp.weixin.qq.com/s?__biz=MzI3MTA0MTk1MA%3D%3D&mid=2652692364&idx=1&sn=806ef08f63d5bf567d421f8808528c22&chksm=f047438bdb7b8b7734f13ec8b0c731b8c3a0085d501b9d6454c92b66b5d973bec5e7bce653ea#rd)｜来源条目：[摘要：Hermes Agent抄袭中国团队代码实锤！被锤后回应：你删号](summaries/摘要：Hermes Agent抄袭中国团队代码实锤！被锤后回应：你删号.md)

- [原文链接](https://mp.weixin.qq.com/s?__biz=Mzg3MTk3NzYzNw%3D%3D&mid=2247506451&idx=1&sn=aec841a1c8c8ec2a99d64536babf7eae&chksm=cf1f5a4baf411e277ea71f2add117a204ea05d54a3884c6c63f8d79ec931d7d3a48694ae7f43#rd)｜《妈耶 HermesAgent直播回应抄没抄国内开发者》｜来源条目：[摘要：妈耶 HermesAgent直播回应抄没抄国内开发者](summaries/摘要：妈耶 HermesAgent直播回应抄没抄国内开发者.md)

## 关联概念

- [Hermes Agent](entities/Hermes Agent.md)

- [Hermes Agent Self-Evolution](entities/Hermes Agent Self-Evolution.md)

- [Evolver](entities/Evolver.md)

- [GEP 协议](concepts/GEP 协议.md)

- [GEPA](concepts/GEPA.md)

- [自我进化 Agent](concepts/自我进化 Agent.md)

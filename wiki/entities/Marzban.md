---
title: Marzban
type: entity
tags: []
status: 审核中
confidence: medium
last_compiled: '2026-04-27'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/4a7d5e113ff1487ab51849196ef528d2
notion_id: 4a7d5e11-3ff1-487a-b518-49196ef528d2
---

## 定义

Marzban 是一个面向代理服务管理场景的面板工具，常见于 Xray/Sing-box 生态，通常被用作中等规模运营场景下的代理面板方案。

## 关键要点

- 在这篇资料中，Marzban 被当作 Remnawave 的直接竞品和迁移来源之一。

- 它代表了一类相对成熟的运营型代理面板，而非单纯的个人快速部署工具。

- 当文章讨论面板迁移、兼容和运营能力时，Marzban 常作为对照基准出现。

## 来源引用

- [摘要：Remnawave：比 3x-ui 更现代的代理面板，俄罗斯团队打造的全自动化管理方案](summaries/摘要：Remnawave：比 3x-ui 更现代的代理面板，俄罗斯团队打造的全自动化管理方案.md)（[原文](https://x.com/dingyi/status/2045743670050353594)）

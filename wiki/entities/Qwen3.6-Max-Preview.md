---
title: Qwen3.6-Max-Preview
type: entity
tags: []
status: 审核中
confidence: high
last_compiled: '2026-04-20'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/1c189a7510d14d988c1826600a56beff
notion_id: 1c189a75-10d1-4d98-8c18-26600a56beff
---

## 定义

Qwen3.6-Max-Preview 是阿里千问 3.6 系列的旗舰预览版模型，主打更强的 Agentic Coding、知识能力与指令遵循表现。

## 关键要点

- 它在文章引用的 Artificial Analysis 榜单中超过 GLM5.1、MiniMax-M2.7 等国产模型。

- 提升最突出的方向是智能体编程，在多个代码库、终端、网页与工具调用基准上表现强势。

- 作为 Preview 版本，它更像正式版前的能力预演，说明阿里希望先释放性能信号，再推进后续产品节奏。

- 它与 Qwen3.6-Plus、Qwen3.6-Flash、Qwen3.6-35B-A3B 共同构成按场景分层的模型矩阵。

## 关联概念

- [SWE-bench Pro](concepts/SWE-bench Pro.md)

- [Terminal-Bench 2.0](entities/Terminal-Bench 2.0.md)

- [SkillsBench](concepts/SkillsBench.md)

- [Qwen3.6-35B-A3B](entities/Qwen3.6-35B-A3B.md)

- [Qwen3.6-Plus](entities/Qwen3.6-Plus.md)

## 来源引用

- [摘要：刚刚，千问最强模型发布，登顶国产最佳](summaries/摘要：刚刚，千问最强模型发布，登顶国产最佳.md)（[原文](https://mp.weixin.qq.com/s?__biz=MjM5MjAyNDUyMA%3D%3D&mid=2651088482&idx=1&sn=f82d4a5697aef837b57ea6353285d4be&chksm=bcff0435172cd02b80b80bc26991b8c0718cb409d76938cfb784f9aa171331118262579e5653#rd)）

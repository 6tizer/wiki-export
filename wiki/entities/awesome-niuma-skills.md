---
title: awesome-niuma-skills
type: entity
tags:
- 知识管理
- 内容自动化
status: 审核中
confidence: medium
last_compiled: '2026-04-18'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/542a481f94d0440ab4ab36ee2555785d
notion_id: 542a481f-94d0-440a-b4ab-36ee2555785d
---

## 定义

awesome-niuma-skills 是一个面向职场高频任务的开源 Skill 仓库，把简历、PRD、面试、会议纪要、标题创作等工作场景整理成 180+ 可复用的技能模板。

## 关键要点

- 它把零散的干活经验封装成结构化 Skill，降低从零启动任务的成本。

- 既可以被人直接阅读照着执行，也可以被 AI 作为外部任务说明加载。

- 它代表了一类“通用职场工作流组件库”式的 Skill 产品形态。

## 来源引用

- [摘要：这个开源牛马Skills180+大合集，专注牛马优雅干活](summaries/摘要：这个开源牛马Skills180+大合集，专注牛马优雅干活.md)（[原文](https://mp.weixin.qq.com/s?__biz=Mzk0MjM4Mjc1Mw%3D%3D&mid=2247491139&idx=1&sn=2171d0f8e6e16fc6ba280948b5afa2c6&chksm=c361e5fb85975dba21cb8a655c8654d851dbbf508e8114d091e09d81824750ab3f13c0de9549#rd)）

## 关联概念

- [Agent Skills](concepts/Agent Skills.md)

- [SKILL.md](concepts/SKILL.md.md)

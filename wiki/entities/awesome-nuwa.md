---
title: awesome-nuwa
type: entity
tags:
- Agent 技能
- Coding Agent
status: 审核中
confidence: high
last_compiled: '2026-04-15'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/b12e24591ed347efb0c03ed087254b69
notion_id: b12e2459-1ed3-47ef-b0c0-3ed087254b69
---

## 定义

**awesome-nuwa** 是一个基于 [女娲.skill](concepts/女娲.skill.md) 批量蒸馏而成的开源 Skill 合集项目，把上百位人物的思维方式整理成可直接安装的 Claude Code Skills。

## 核心特征

- 将人物认知蒸馏从单次实验扩展为**规模化仓库**

- 覆盖哲学家、科学家、商业领袖与中国企业家等多类人物

- 每个条目都是可安装的 Skill，而不是语录摘抄或人物简介

- 证明了心智模型蒸馏可以被复用、组合，并形成生态飞轮

## 关键要点

- 它是 [女娲.skill](concepts/女娲.skill.md) 的生态放大器

- 它让“想用谁的脑子想问题，装谁就行”变成可执行工作流

- 它与 [达尔文.skill](entities/达尔文.skill.md) 共同构成“生成 + 优化”的 Skill 生命周期

## 来源引用

- 《Github 上的硅基生物 —— 一手达尔文、一手女娲 ，先造人后进化》｜微信文章｜原文链接：[https://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ==&mid=2247493022&idx=1&sn=624a417e7c20389f39d7c12d68db105c&chksm=c03f662e4b855b2d7f8982c96394e2f3f90f128f81904761993d26762b66605dac072cafc190#rd](https://mp.weixin.qq.com/s?__biz=MzkyNDYyODg0MQ%3D%3D&mid=2247493022&idx=1&sn=624a417e7c20389f39d7c12d68db105c&chksm=c03f662e4b855b2d7f8982c96394e2f3f90f128f81904761993d26762b66605dac072cafc190#rd)｜源文章：Github 上的硅基生物 —— 一手达尔文、一手女娲 ，先造人后进化

## 关联概念

- [女娲.skill](concepts/女娲.skill.md)

- [心智模型蒸馏](concepts/心智模型蒸馏.md)

- [达尔文.skill](entities/达尔文.skill.md)

---
title: Readwise
type: entity
tags:
- 知识管理
status: 审核中
confidence: medium
last_compiled: '2026-04-23'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/b7b217628df8412295b4d38b5d3dd718
notion_id: b7b21762-8df8-4122-95b4-d38b5d3dd718
---

## 定义

Readwise 是一个用于聚合阅读高亮、剪藏内容与学习素材的知识整理工具，常被用来把网页、播客、Kindle 等来源的内容自动同步进个人知识库。

## 关键要点

- 适合作为多源阅读素材的统一汇集层

- 能把“收藏”进一步推进为“可回顾、可整理、可再利用”的知识资产

- 在 Obsidian 工作流中常与剪藏、摘要、归档流程串联使用

- 其意义在于把分散输入转成可持续处理的待整理队列

## 来源引用

- [摘要：Obsidian + AI：8 个值得搭的自动化工作流，记得收藏！](summaries/摘要：Obsidian + AI：8 个值得搭的自动化工作流，记得收藏！.md)（[原文](https://x.com/xiangxiang103/status/2046845848244617468)）

## 关联概念

- [Obsidian](entities/Obsidian.md)

- [Obsidian Web Clipper](entities/Obsidian Web Clipper.md)

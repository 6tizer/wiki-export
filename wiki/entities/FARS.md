---
title: FARS
type: entity
tags:
- 模型评测
- AI 产品
status: 审核中
confidence: medium
last_compiled: '2026-04-27'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/9ff4998700f947018708c96e3d8ba4f4
notion_id: 9ff49987-00f9-4701-8708-c96e3d8ba4f4
---

## 定义

FARS（Full Auto Research System）是日行迹开发的自动化科研系统，旨在让 AI 智能体独立完成从 idea 生成到论文写作的完整科研流程。

## 关键要点

- 系统架构复杂，总成本约 18 万美元

- 在 Stanford Agentic Reviewer（SAR）评测中，得分低于简单使用 Claude Code 的方案（平均低约 0.4 分）

- 与 Codex（GPT 5.4）直接生成的论文得分接近

- 其高成本与复杂架构并未带来显著的质量优势，表明 Auto Research 可能不需要过度复杂的系统设计

## 来源引用

- [摘要：200刀打败18万美元FARS，我们离真正的Auto Research还有多远？](summaries/摘要：200刀打败18万美元FARS，我们离真正的Auto Research还有多远？.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzIwMTc4ODE0Mw%3D%3D&mid=2247719869&idx=1&sn=286785641a7ed52f6a793ada026e99b7&chksm=97fd86517cd8ac029041769d3854be3aa40b9cfb029f9227e4c44dddc772f6182f320b544355#rd)）

## 关联概念

- [Auto Research](concepts/Auto Research.md)

- [autoresearch](entities/autoresearch.md)

- [Stanford Agentic Reviewer](entities/Stanford Agentic Reviewer.md)

- [Agentic Review 偏差](concepts/Agentic Review 偏差.md)

- [Claude Code](entities/Claude Code.md)

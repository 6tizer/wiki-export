---
title: Physical Intelligence
type: entity
tags:
- AI 产品
- 训练/微调
status: 草稿
confidence: medium
last_compiled: '2026-04-17'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/c1a428e400b84429a681857884ea8f9e
notion_id: c1a428e4-00b8-4429-a681-857884ea8f9e
---

### 定义

Physical Intelligence 是聚焦通用机器人智能的研究与产品团队，围绕 VLA 路线持续推进机器人模型的泛化能力。

### 关键要点

- 文章将其定位为 π0.7 的发布方和核心研究主体

- 团队延续了从 RT-2 到 π 系列的 VLA 路线判断

- 其研究重点之一是让机器人系统在更多异构数据上获得更强的组合泛化

### 来源引用

- [π0.7发布，VLA押出了机器人的GPT-3时刻](https://mp.weixin.qq.com/s?__biz=MzIzNjc1NzUzMw%3D%3D&mid=2247884140&idx=2&sn=a26f0706c04bc3c8dd53f152e4133e38&chksm=e98d64f1cb9afef869d1774c4600f92227b8c887705347a50408a07afa2befe699960bb212b7#rd)｜量子位｜2026-04-17｜源页面：π0.7发布，VLA押出了机器人的GPT-3时刻

### 备注

这是公司/团队实体页面，可继续累计其后续模型与研究动态。

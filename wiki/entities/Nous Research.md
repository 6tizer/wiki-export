---
title: Nous Research
type: entity
tags:
- 商业/生态
status: 审核中
confidence: high
last_compiled: '2026-04-20'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/ce51ec2e77b844faa54185df559665ac
notion_id: ce51ec2e-77b8-44fa-a541-85df559665ac
---

## 定义

Nous Research 是一家以开源为核心策略的 AI 基础设施公司，覆盖模型架构、数据合成、微调、推理与模型接入平台等方向，并围绕 Hermes 系列模型与 Nous Portal 构建产品化能力。

## 关键要点

- 公司的核心竞争力在于“研究 + 开源 + 产品入口”三位一体：既做模型研究，也开放权重、数据集与训练细节。

- Hermes 系列模型是其代表性产品，帮助公司在开源模型社区建立品牌心智。

- Nous Portal 承担商业化入口，让用户通过统一账户访问多模型与第三方工具。

- 从这篇文章看，Nous Research 的定位不只是做模型，而是在尝试搭建面向 Agent 场景的模型与工具分发层。

## 来源引用

- [摘要：7 周 128K stars 爆火的 Hermes Agent，背后是 NousResearch 这群 web3 来的加密原住民](summaries/摘要：7 周 128K stars 爆火的 Hermes Agent，背后是 NousResearch 这群 web3 来的加密原住民.md)（[原文](https://x.com/Xudong07452910/status/2050493896237396410)）

- [摘要：上次在 Hermes 里小米模型免费用，登录他们后台一看，这不是模型中转贩子吗？仔细一瞧居然有自研模型，再仔细研究一下，发现这家公司真不简单！](summaries/摘要：上次在 Hermes 里小米模型免费用，登录他们后台一看，这不是模型中转贩子吗？仔细一瞧居然有自研模型，再仔细研究一下，发现这家公司真不简单！.md)（[原文](https://x.com/laogui/status/2044930449538191713)）

## 关联概念

- [Hermes Agent](entities/Hermes Agent.md)

- [Nous Portal](entities/Nous Portal.md)

- [Nous Tool Gateway](entities/Nous Tool Gateway.md)

- [DisTrO](concepts/DisTrO.md)

- [Psyche Network](entities/Psyche Network.md)

- [YaRN](concepts/YaRN.md)

- [中立对齐](concepts/中立对齐.md)

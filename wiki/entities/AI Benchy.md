---
title: AI Benchy
type: entity
tags:
- 模型评测
- 加密资产
- AI 产品
status: 审核中
confidence: medium
last_compiled: '2026-04-16'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/dd1fccedddfa456ba5f7bc5f14fb993b
notion_id: dd1fcced-ddfa-456b-a5f7-bc5f14fb993b
---

## 定义

AI Benchy 是一个偏向真实工作流场景的民间 AI 评测工具，更强调指令遵循、成本效率与自动化任务中的实用表现，而不是只看厂商官方跑分。

## 关键要点

- 适合评估开发者和自动化工作流更关心的实际可用性

- 重点衡量指令遵循度、真实性价比与实际执行效率

- 在本文中被用来辅助判断 Elephant 的 token 效率与指令遵循表现

## 来源引用

- [原文链接](https://mp.weixin.qq.com/s?__biz=MzA4MTQ4NjQzMw%3D%3D&mid=2652801118&idx=1&sn=f40df183d831b17f5f8dcc100fb9c9a3&chksm=85f152316f89ca9f6eaec156d55adaa7df30ec97ff2d436b45e2f918da524206b3b000e6dc21#rd)｜《匿名模型“大象”搅局OpenRouter：100B参数冲到热榜第一，实测结果如何》｜源文章：匿名模型“大象”搅局OpenRouter：100B参数冲到热榜第一，实测结果如何

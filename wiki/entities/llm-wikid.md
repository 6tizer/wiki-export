---
title: llm-wikid
type: entity
tags:
- 知识管理
- Coding Agent
status: 草稿
confidence: medium
last_compiled: '2026-04-15'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/ffc46f07b27541cea0583ae70997692f
notion_id: ffc46f07-b275-41ce-a058-3ae70997692f
---

## 定义

llm-wikid 是 Shann Holmberg 开源的知识层框架与仓库，用于把原始资料编译为结构化 Wiki，并配合 Agent 执行 ingest、query、explore、lint 等操作。

## 关键要点

- 项目以 raw/、wiki/、templates/ 和 [CLAUDE.md](http://claude.md/) 等目录/规范文件为核心结构。

- 它强调 20 分钟快速搭建、持续摄取与持续查询的工作流。

- 适合内容创作者、个人知识库和小团队知识协作场景。

- 它是文章中 AI Knowledge Layer 方案的具体开源实现。

## 来源引用

- [原文链接](https://x.com/shannholmberg/status/2044111115878326444)｜《AI Knowledge Layer (and why your agents are useless without it)》｜源文章：LLM Wikid：让你的 AI Agent 真正「认识你」的知识层架构

- [项目仓库](https://github.com/shannhk/llm-wikid.git)

## 关联概念

- [Karpathy LLM Wiki 方法论](concepts/Karpathy LLM Wiki 方法论.md)

- [Obsidian Web Clipper](entities/Obsidian Web Clipper.md)

- [Context Engineering](concepts/Context Engineering.md)

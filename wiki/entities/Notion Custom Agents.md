---
title: Notion Custom Agents
type: entity
tags:
- Agent 框架
- 工作流
status: 草稿
confidence: high
last_compiled: '2026-04-12'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/40b9fb03f87a4ee5a0f9c2b4d8f43dde
notion_id: 40b9fb03-f87a-4ee5-a0f9-c2b4d8f43dde
---

## 定义

Notion Custom Agents 是 Notion 提供的可定制 Agent 产品形态，能够在工作区内读取数据库、响应触发器，并把结果写回到现有流程中。

## 核心要点

- 核心优势是直接建立在 Notion 数据库和页面体系之上

- 适合把任务分派、会议后处理、报告生成等流程嵌入已有工作流

- 价值不在单个 Agent 的聪明程度，而在于与工作区数据模型的深度耦合

## 关联概念

- [Agent Harness](concepts/Agent Harness.md)

- [Coding Agent](concepts/Coding Agent.md)

- [软件工厂](concepts/软件工厂.md)

- [Notion 风格 Markdown](concepts/Notion 风格 Markdown.md)

- [MCP 协议](concepts/MCP 协议.md)

- [Evals](concepts/Evals.md)

## 来源引用

- [原文链接](https://x.com/kplikethebird/status/2042260326058512598)｜《Every 用四个 Notion AI Agent 运营一家 25 人公司》｜来源页：Every 用四个 Notion AI Agent 运营一家 25 人公司

- [原文链接](https://mp.weixin.qq.com/s?__biz=Mzg5NTc0MjgwMw%3D%3D&mid=2247523808&idx=1&sn=0a6aea0c1f3f9c5c6e86eff9592fd30b&chksm=c17464df982ee29fa92c7272b71d87b2dc6c54287a5fd8e61657920ebeb34565f22f3d893ff4#rd)｜《Notion Custom Agents复盘：三年重写5次，Notion 历史上最成功的新功能之一》｜源文章：Notion Custom Agents复盘：三年重写5次，Notion 历史上最成功的新功能之一

---
title: Andrej Karpathy
type: entity
tags:
- 知识管理
- AI 设计
- 笔记工具
status: 审核中
confidence: medium
last_compiled: '2026-04-23'
source_tags: ''
source_article_url: https://www.notion.so/34b701074b6881bf90d2d26ee121e128
notion_url: https://www.notion.so/Tizer/a03566ba4bf640bfa7cdb04b02d09c56
notion_id: a03566ba-4bf6-40bf-a7cd-b04b02d09c56
---

## 定义

Andrej Karpathy 是这篇条目中的核心人物对象；该项目尝试把其公开输出材料整理为一组可复用的人格与风格文件，用于构建具备稳定表达特征的 AI 代理画像。

## 关键要点

- 这次编译对象不是单篇观点，而是围绕个人长期公开语料构建的“人格资料包”。

- 源推文给出的材料覆盖博客、YouTube 转录、仓库 README 和推文，说明其公开语料密度高、适合做 persona distillation。

- 在 Wiki 语境下，Andrej Karpathy 更适合作为 **entity** 管理，再与相关 summary 和概念页建立引用关系。

## 来源引用

- [摘要：The @karpathy SOUL is now live:](summaries/摘要：The @karpathy SOUL is now live-.md)（[原文](https://x.com/aeonframework/status/2046964241211507055)）

- 摘要：Karpathy AI Ascent 2026 三大新视野（[原文](https://x.com/karpathy/status/2049903821095354523)）

## 关联概念

- [SOUL.md](concepts/SOUL.md.md)

- [STYLE.md](concepts/STYLE.md.md)

- [MEMORY.md](concepts/MEMORY.md.md)

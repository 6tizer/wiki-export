---
title: we-mp-rss
type: entity
tags:
- 知识管理
- 开发工具
status: 草稿
confidence: medium
last_compiled: '2026-04-17'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/764f9c3a2fae415a8b0dee8ec31a6576
notion_id: 764f9c3a-2fae-415a-8b0d-ee8ec31a6576
---

## 定义

we-mp-rss 是一个微信公众号 RSS 订阅与文章导出工具，可将公众号文章转换为 Markdown 文档，并生成可供阅读器订阅的 RSS 源。

## 关键要点

- 它把原本封闭在微信公众号内的内容转成更易保存、检索和再利用的文档格式。

- 它既可作为个人归档工具，也可作为后续知识编译流程的原始资料入口。

- 在本文语境里，它为后续本地化保存和知识图谱处理提供了上游数据接口。

## 关联概念

- [Karpathy LLM Wiki 方法论](concepts/Karpathy LLM Wiki 方法论.md)

- [Graphify](entities/Graphify.md)

## 来源引用

- [原文链接](https://mp.weixin.qq.com/s?__biz=MzA3OTEwNjUwMw%3D%3D&mid=2451582189&idx=1&sn=5f669d3b8e480a52044f20c318c907f5&chksm=8989bab920c529851a230e42f6a695b110e2da878b190d07b8792b21b3e41274d73bfc02c081#rd)｜《用 werss-cli 将公众号历史文章重新保存到本地，再用 graphify 把 100 多篇文章变成知识图谱》｜源文章：用 werss-cli 将公众号历史文章重新保存到本地，再用 graphify 把 100 多篇文章变成知识图谱

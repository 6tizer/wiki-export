...
- 基于 @badlogicgames 开发的 Pi 框架构建，Electron 桌面应用为主界面，配有 iOS 伴侣应用（通过 Tailscale 远程监控）
...
- Agent Lifecycle Management：支持定时任务、Deferred Resume（延迟唤醒）、队列跟进，让 Agent 自主管理运行节奏

- Auto Mode 延续模式：注入 review + continuation prompt 让 Agent 自主判断是否继续工作

- 内嵌 Excalidraw 用于向 Agent 传递手绘图和截图标注，内置语音输入（基于 OpenAI dictation）

- 内嵌知识库：Skills + [AGENTS.md](http://agents.md/) + 自由笔记，采用纯 Markdown 文件夹方案（Git 仓库 + 自动同步引擎，无 RAG），参考 Karpathy 的 Obsidian 知识库思路
...
- [摘要：Personal Agent](summaries/摘要：Personal Agent.md)（[X 原文](https://x.com/trashpandaemoji/status/2048026069375029267)）

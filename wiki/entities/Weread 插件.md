---
title: Weread 插件
type: entity
tags:
- 知识管理
status: 审核中
confidence: medium
last_compiled: '2026-04-23'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/b7471044950d4c9c9fd2addd90bf71b9
notion_id: b7471044-950d-4c9c-9fd2-addd90bf71b9
---

## 定义

Weread 插件是连接微信读书与 Obsidian 的同步工具，用于把微信读书中的划线和笔记导入本地知识库，方便后续由 AI 继续整理、提炼和改写。

## 关键要点

- 把阅读笔记从阅读应用搬运到可编排的知识库环境

- 适合与内容模板配合，把读书笔记进一步转成文章、播客或分享提纲

- 在整条工作流中承担“阅读素材同步器”的角色

- 它强化的是从输入素材到内容产出的可重复链路

## 来源引用

- [摘要：Obsidian + AI：8 个值得搭的自动化工作流，记得收藏！](summaries/摘要：Obsidian + AI：8 个值得搭的自动化工作流，记得收藏！.md)（[原文](https://x.com/xiangxiang103/status/2046845848244617468)）

## 关联概念

- [Obsidian](entities/Obsidian.md)

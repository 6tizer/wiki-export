---
title: WorldArena
type: entity
tags:
- LLM
status: 审核中
confidence: high
last_compiled: '2026-04-23'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/5e1d8bbd027b4ad08ccfa7cb01faece6
notion_id: 5e1d8bbd-027b-4ad0-8ccf-a7cb01faece6
---

## 定义

WorldArena 是一个用于评估世界模型对现实世界运动、时序变化与物理一致性理解能力的 benchmark，文章将其视为“预测世界”能力的重要代表。

## 关键要点

- 更偏向 world model 维度的评测。

- 关注 Motion Quality、Flow Score、Motion Smoothness 等运动相关指标。

- 适合观察模型是否真正学会了对动态世界进行连续建模与预测。

## 来源引用

- [摘要：拒绝透露姓名，却拿了两个世界第一：具身圈大佬们都在打听它是谁](summaries/摘要：拒绝透露姓名，却拿了两个世界第一：具身圈大佬们都在打听它是谁.md)

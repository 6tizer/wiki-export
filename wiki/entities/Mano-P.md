---
title: Mano-P
type: entity
tags:
- 多模态
- 模型部署
- AI 产品
status: 审核中
confidence: high
last_compiled: '2026-04-13'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/b4f52fe249754cf38cbcf28bc9b54dbe
notion_id: b4f52fe2-4975-4cf3-8cbc-f28bc9b54dbe
---

> **🧭** **定义**：Mano-P 是明略科技推出的面向端侧设备的 GUI-VLA 智能体模型，强调纯视觉界面理解、本地运行、零上云与跨桌面软件操作能力。

### 关键要点

- 定位为可直接操作桌面软件、网页界面的纯视觉 GUI Agent，不依赖传统 API 对接或浏览器专用协议。

- 提供从 Skill、模型到训练方法的三阶段开源路线，兼顾即用性、可定制性与生态扩展。

- 支持在 Apple M4 设备上本地推理，突出隐私保护、离线执行与低门槛部署。

- 文章将其视为 GUI Agent 从“爪”走向“手”的代表性产品。

### 来源引用

- 《全球第一，13个SOTA！我们找到了龙虾界掌管GUI的神》｜作者：机器之心｜发布：2026-04-13T11:58:00.000+08:00｜原文：[文章链接](https://mp.weixin.qq.com/s?__biz=MzA3MzI4MjgzMw%3D%3D&mid=2651027314&idx=1&sn=5f962ab9416c2eee0c4769a51057ced7&chksm=85b9064525b8127699bc59956dff0aba0eaabb800d3001d9595f29adf5055370ff3d7259f0d8#rd)｜源页面：全球第一，13个SOTA！我们找到了龙虾界掌管GUI的神

### 关联概念

- GUI-VLA

- Mano-CUA Skill

- Mano-Action 双向自增强学习框架

- GSPruning

- Personalized AI

---
title: AlphaBrain Platform
type: entity
tags:
- LLM
- 商业/生态
status: 审核中
confidence: high
last_compiled: '2026-04-23'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/674048f5c3be4ad5b6d2288850a4db7b
notion_id: 674048f5-c3be-4ad5-b6d2-288850a4db7b
---

## 定义

AlphaBrain Platform 是智平方开源的一站式具身智能模型社区与研发平台，目标是把世界模型、类脑模型、强化学习后训练、持续学习和统一 Benchmark 等关键能力整合到同一套可复现工具链里。

## 关键要点

- 它强调的不只是“开源一个模型”，而是开源从训练、评测到场景化落地的完整基础设施。

- 平台预置多种主流世界模型 Backbone，并支持统一配置与切换，便于做横向评测。

- 它把 RL Token、NeuroVLA、持续学习等路线纳入同一平台，降低研究和复现门槛。

- 从产业意义看，它是智平方争夺具身智能开源标准制定权的重要载体。

## 来源引用

- [摘要：特斯拉开源硬件，中国公司回应来了：直接把机器人大脑开源了](summaries/摘要：特斯拉开源硬件，中国公司回应来了：直接把机器人大脑开源了.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzIzNjc1NzUzMw%3D%3D&mid=2247885437&idx=1&sn=6a4c424526417657d528b8bfa9d0c200&chksm=e9963f58459d6d389133ac94bd91c6f2c51c8c7cc4fba208ced0e1d73052dc4d5db0a7661e16#rd)）

## 关联概念

- [智平方](entities/智平方.md)

- [RL Token](concepts/RL Token.md)

- [NeuroVLA](entities/NeuroVLA.md)

- [可插拔世界模型架构（WA）](concepts/可插拔世界模型架构（WA）.md)

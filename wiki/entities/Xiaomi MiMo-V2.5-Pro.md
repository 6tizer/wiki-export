---
title: Xiaomi MiMo-V2.5-Pro
type: entity
tags:
- LLM
- Coding Agent
status: 草稿
confidence: high
last_compiled: '2026-04-28'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/69d649f2b85440cdb2856da4b79f17fc
notion_id: 69d649f2-b854-40cd-b285-6da4b79f17fc
---

## 定义

Xiaomi MiMo-V2.5-Pro 是小米 MiMo 系列在 2026 年 4 月发布的高阶模型版本，主打长时程 agentic 任务、复杂软件工程与高强度工具调用场景。

## 关键要点

- 官方将其定位为当前最强的 MiMo 模型，强调在复杂软件工程与长时程任务上的跃升。

- 在官方案例中，该模型可在合适的 harness 环境下持续完成 1000+ 次工具调用，支撑数小时级的自治执行。

- 官方用 SysY 编译器、视频编辑器、模拟电路优化等任务展示其规划、自我纠错和长程一致性能力。

- 产品层面已在 API Platform、AI Studio 等入口上线，且官方表示价格不变。

## 关联概念

- [Xiaomi MiMo-V2.5](entities/Xiaomi MiMo-V2.5.md)

- [MiMo Coding Bench](concepts/MiMo Coding Bench.md)

- [Claw-Eval](entities/Claw-Eval.md)

- [Harness Engineering](concepts/Harness Engineering.md)

- [Claude Code](entities/Claude Code.md)

- [CC Switch](entities/CC Switch.md)

## 来源引用

- [摘要：Xiaomi MiMo-V2.5 Series: Pushing Open-Source Agents Forward](summaries/摘要：Xiaomi MiMo-V2.5 Series- Pushing Open-Source Agents Forward.md)（[原文](https://x.com/XiaomiMiMo/status/2046988157888209365)）

- [摘要：实测小米MiMo-V2.5-Pro，这可能是目前国内最适合Claude Code的新模型。](summaries/摘要：实测小米MiMo-V2.5-Pro，这可能是目前国内最适合Claude Code的新模型。.md)（[原文](https://x.com/Khazix0918/status/2047526414161965523)）

- [摘要：4.3 小时写完一个编译器！小米凌晨开源 MiMo-V2.5，反超 DeepSeek V4](summaries/摘要：4.3 小时写完一个编译器！小米凌晨开源 MiMo-V2.5，反超 DeepSeek V4.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzU3NTg5MjU1Mw%3D%3D&mid=2247497120&idx=1&sn=adb691fd7cb52fb63f3ab6f69fa5011b&chksm=fcc107b09a5c6e5f3ac7df70dcd88c86ac115110d41477790ef5d97e8fc7336b219c3d6bb535#rd)）

- [摘要：小米 MiMo-V2.5 系列开源 & Orbit 百万亿 Token 计划启动](summaries/摘要：小米 MiMo-V2.5 系列开源 & Orbit 百万亿 Token 计划启动.md)（[原文](https://mp.weixin.qq.com/s?__biz=Mzk3NTkxMTM2NA%3D%3D&mid=2247484681&idx=1&sn=60241f47e569b8105e66bf704cd4c98b&chksm=c50137dcd08f21a4eb67a631268f2b59dcc62a811ba5f31be372f09d38e5300b8474690cc111#rd)）

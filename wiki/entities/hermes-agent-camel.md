---
title: hermes-agent-camel
type: entity
tags:
- Agent 安全
- Harness 工程
status: 审核中
confidence: medium
last_compiled: '2026-04-23'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/b358229d5f304e8aa37392fe4f0c526b
notion_id: b358229d-5f30-4e8a-a373-92fe4f0c526b
---

## 定义

hermes-agent-camel 是一个基于 Hermes Agent 的社区分叉项目，在原有自进化 Agent 循环之上整合了 CaMeL 信任边界机制，用来增强生产环境中的安全控制。

## 核心要点

- 把 CaMeL 的信任边界原则嵌入 Hermes 运行时，重点缓解提示注入与越权调用风险

- 保留 Hermes Agent 原有的自我改进、技能沉淀和跨会话工作能力

- 适合希望把 Hermes 更稳妥地用于真实生产任务的使用者

- 属于 Hermes 生态里“安全增强型分支”的代表项目

## 来源引用

- [摘要：Hermes Agent 生态继续疯涨](summaries/摘要：Hermes Agent 生态继续疯涨.md)（[原文](https://x.com/GitTrend0x/status/2046526941038280756)）

- [摘要：Hermes Agent 生态继续狂飙，又卷出一大批全新进化体！](summaries/摘要：Hermes Agent 生态继续狂飙，又卷出一大批全新进化体！.md)（[原文](https://x.com/GitTrend0x/status/2045837379827896407)）

## 关联概念

- [Hermes Agent](entities/Hermes Agent.md)

- [Hermes Atlas](entities/Hermes Atlas.md)

- [CaMeL 信任边界](concepts/CaMeL 信任边界.md)

...
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/e2d64655509249e0bfb2002d206b2359
notion_id: e2d64655-5092-49e0-bfb2-002d206b2359
---

## 定义

Claude Code 是 Anthropic 推出的**命令行 AI 编程助手**，直接运行在终端里。不需要打开 IDE 或安装插件，可自主执行 Shell 命令、递归修改多个文件、修复 Bug——是真正的 Agent 模式而非代码补全。

## 当前规模（2026 年 3 月）

- GitHub Stars：40,000+

- 占全球 GitHub 提交的约 **4%**，日提交量超过 13.5 万次

- 创作者 Boris 公开表示上个月 100% 的代码贡献都由 Claude Code 自己写

## 典型用法

- 与 Cursor 配合使用：Cursor 日常编写 + Claude Code 处理复杂任务

- 国内用户可通过阿里百炼 API 代替官方订阅，支持 Kimi、精准 GLM、通义千问等模型
...
| Claude Code | 终端 CLI | 深度 Agent，自主执行命令 |

| Cursor | IDE | 日常编写+补全，上下文感知强 |

| Windsurf | IDE | 轻量易用，适合入门 |

| Cline/OpenCode | 插件/终端 | 开源替代 |
...
- [摘要：ScreenKite 1.0：让 AI 替你剪视频的 macOS 录屏神器](summaries/摘要：ScreenKite 1.0：让 AI 替你剪视频的 macOS 录屏神器.md)（[原文](https://x.com/screenkite_com/status/2046052920794423650)）
...
- [摘要：The Agency：给你的 AI 编程工具配齐 144 个专属打工人](summaries/摘要：The Agency：给你的 AI 编程工具配齐 144 个专属打工人.md)（@AYi_AInotes，X书签）— 补充 Claude Code 作为角色型 Agent 配置宿主的典型用法

- [摘要：GEO-SEO Claude Code Skill：用 AI Agent 给网站做「AI 搜索引擎优化」](summaries/摘要：GEO-SEO Claude Code Skill：用 AI Agent 给网站做「AI 搜索引擎优化」.md)（GitHubDaily，X书签）— 补充 Claude Code 从编程助手扩展到 GEO/SEO 审计工作流的执行宿主角色

- [摘要：手把手教你低成本用上 Claude Code：VPS + 住宅 IP + 订阅全流程](summaries/摘要：手把手教你低成本用上 Claude Code：VPS + 住宅 IP + 订阅全流程.md)（X书签，[原文链接](https://x.com/aehyok/status/2031982094914896184)）

- [摘要：深度使用半年，我总结了 Claude Code 的架构、治理与工程实践](summaries/摘要：深度使用半年，我总结了 Claude Code 的架构、治理与工程实践.md)（X书签，[原文链接](https://x.com/HiTw93/status/2032091246588518683)）
...
- [原文链接](http://github.com/msitarzewski/agency-agents)｜《agency-agents：用 147 个 Markdown 文件，搭建你的零成本 AI 专业团队》｜来源条目：[摘要：agency-agents：用 147 个 Markdown 文件，搭建你的零成本 AI 专业团队](summaries/摘要：agency-agents：用 147 个 Markdown 文件，搭建你的零成本 AI 专业团队.md)
...
- [原文链接](https://x.com/the_smart_ape/status/2043262727922053128)｜《claude code + obsidian + skill graph. how to build a local research engine?》｜来源条目：[摘要：claude code + obsidian + skill graph. how to build a local research engine?](summaries/摘要：claude code + obsidian + skill graph. how to build a local research engine.md)
...
- [原文链接](https://x.com/gerrox/status/2039137055746543860)｜《有人修复了泄露的 Claude Code 源码，让它真正能跑起来》｜来源条目：[摘要：有人修复了泄露的 Claude Code 源码，让它真正能跑起来](summaries/摘要：有人修复了泄露的 Claude Code 源码，让它真正能跑起来.md)

- [原文链接](https://x.com/gerrox/status/2039137055746543860)｜《Claude Code 源码泄露深度解析：一个开发者如何让它重新编译运行》｜来源条目：[摘要：Claude Code 源码泄露深度解析：一个开发者如何让它重新编译运行](summaries/摘要：Claude Code 源码泄露深度解析：一个开发者如何让它重新编译运行.md)

- [原文链接](https://x.com/gerrox/status/2039137055746543860)｜《Claude Code 源码泄露始末：一个开发者如何让它真正跑起来，并拆解其内部架构》｜来源条目：[摘要：Claude Code 源码泄露始末：一个开发者如何让它真正跑起来，并拆解其内部架构](summaries/摘要：Claude Code 源码泄露始末：一个开发者如何让它真正跑起来，并拆解其内部架构.md)
...
- 《有人修复了泄露的 Claude Code 源码，让它真正能跑起来》｜X书签文章｜原文链接：[https://x.com/gerrox/status/2039137055746543860](https://x.com/gerrox/status/2039137055746543860)｜来源条目：[摘要：有人修复了泄露的 Claude Code 源码，让它真正能跑起来](summaries/摘要：有人修复了泄露的 Claude Code 源码，让它真正能跑起来.md)
...
- [原文链接](https://x.com/claudeai/status/2044095086460309790)｜《Now in research preview: routines in Claude Code.》｜来源条目：[摘要：Now in research preview: routines in Claude Code.](summaries/摘要：Now in research preview- routines in Claude Code.md)
...
- [原文链接](https://mp.weixin.qq.com/s?__biz=MzYyMzc4NjU0Mg%3D%3D&mid=2247485518&idx=1&sn=32379f84dd600df7b002ea9070cc38fd&chksm=feee7cfd7f2988db5205c021092c341fa9b97c0c0d9c8d02c4394af0d93862d6e968ab042e31#rd)｜《Claude Code 全屏渲染来了：长对话不再闪烁，还能用鼠标！》｜来源条目：[摘要：Claude Code 全屏渲染来了：长对话不再闪烁，还能用鼠标！](summaries/摘要：Claude Code 全屏渲染来了：长对话不再闪烁，还能用鼠标！.md)
...
- [原文链接](https://x.com/rohit4verse/status/2044090667274776877)｜《5 pipelines I'd sell today using Claude Code (none of them are coding)》｜来源条目：[摘要：5 pipelines I'd sell today using Claude Code (none of them are coding)](summaries/摘要：5 pipelines I'd sell today using Claude Code (none of them are coding).md)
...
- [原文链接](https://mp.weixin.qq.com/s?__biz=MzA4NzgzMjA4MQ%3D%3D&mid=2453482874&idx=1&sn=b1e7507f4fbee02e96dec2090f95705a&chksm=86eaa21978d6f608aee3fb0293d9272c137637fe815a07ee4d496b7f44b5dcd45ca37856e05c#rd)｜《Claude Opus 4.7 配 Lovart：全套品牌设计只需点击 2 次，设计师可能不太需要了》｜来源条目：[摘要：Claude Opus 4.7 配 Lovart：全套品牌设计只需点击 2 次，设计师可能不太需要了](summaries/摘要：Claude Opus 4.7 配 Lovart：全套品牌设计只需点击 2 次，设计师可能不太需要了.md)
...
- [摘要：In the next version of Claude Code](summaries/摘要：In the next version of Claude Code.md)（[原文](https://x.com/bunjavascript/status/2045300183479865602)）
...
- [摘要：Claude Code 就质量下滑发布事后复盘并重置配额](summaries/摘要：Claude Code 就质量下滑发布事后复盘并重置配额.md)（[原文](https://x.com/ClaudeDevs/status/2047371123185287223)）
...
- [摘要：wshobson/agents：用 72 个插件把 Claude Code 变成 AI 开发部队](summaries/摘要：wshobson-agents：用 72 个插件把 Claude Code 变成 AI 开发部队.md)（GitHub_Daily，2026-03-08）— 展示 Claude Code 如何通过插件、技能包和编排器扩展成多角色开发体系

- [摘要：用 Claude Code 搭一套 BTC 合约交易信号系统，每小时推送到 Telegram](summaries/摘要：用 Claude Code 搭一套 BTC 合约交易信号系统，每小时推送到 Telegram.md)（zeroxmin，X书签）— 展示 Claude Code 作为一次性代码生成器，把分析流程固化为稳定脚本

- [摘要：LangChain CEO：编程 Agent 正在重塑工程、产品与设计团队的协作方式](summaries/摘要：LangChain CEO：编程 Agent 正在重塑工程、产品与设计团队的协作方式.md)（Harrison Chase，X书签）— 将 Claude Code 放在编程 Agent 改变组织分工的语境中讨论

- [摘要：Context Hub：吴恩达开源的 Coding Agent「文档新鲜度」解决方案](summaries/摘要：Context Hub：吴恩达开源的 Coding Agent「文档新鲜度」解决方案.md)（AndrewYNg，X书签）— 体现 Claude Code 与运行时文档补充工具的协同方式
...
- [摘要：用 Karpathy 的 autoresearch 方法，让你的 Claude Skill 自动进化](summaries/摘要：用 Karpathy 的 autoresearch 方法，让你的 Claude Skill 自动进化.md)（X书签，[原文链接](https://x.com/MinLiBuilds/status/2034533228162187444)）

- [摘要：Unusual Whales MCP：让 Claude 和 Cursor 直接调用专业美股交易数据](summaries/摘要：Unusual Whales MCP：让 Claude 和 Cursor 直接调用专业美股交易数据.md)（X书签，[原文链接](https://x.com/oragnes/status/2034570577667850436)）

- [摘要：Moss Trade Bot Factory：用自然语言创建你的 AI 加密交易员，$10k 模拟盘跑出 $2.4M 利润](summaries/摘要：Moss Trade Bot Factory：用自然语言创建你的 AI 加密交易员，$10k 模拟盘跑出 $2.4M 利润.md)（X书签，[原文链接](https://x.com/zeroxmin/status/2034607947171717158)）
...
- 《Claude Code 的七层记忆体系：从亚毫秒级缓存到「梦境」式整合》｜X书签文章｜原文链接：[https://x.com/shao__meng/status/2039153466229088451](https://x.com/shao__meng/status/2039153466229088451)｜来源条目：摘要：Claude Code 的七层记忆体系：从亚毫秒级缓存到「梦境」式整合
...
- [原文链接](https://x.com/troyhua/status/2039052328070734102)｜《Claude Code 的七层记忆架构：从毫秒级 Token 裁剪到「睡眠整理」系统》｜来源条目：摘要：Claude Code 的七层记忆架构：从毫秒级 Token 裁剪到「睡眠整理」系统

- [原文链接](https://x.com/shao__meng/status/2039153466229088451)｜《Claude Code 的七层记忆体系：从亚毫秒级缓存到「梦境」式整合》｜来源条目：摘要：Claude Code 的七层记忆体系：从亚毫秒级缓存到「梦境」式整合

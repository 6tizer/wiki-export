---
title: PaperClaw
type: entity
tags:
- Agent 框架
- 记忆系统
status: 审核中
confidence: medium
last_compiled: '2026-04-15'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/f8de41b8819e4174a36741b8138489f7
notion_id: f8de41b8-819e-4174-a367-41b8138489f7
---

## 定义

PaperClaw 是面向科研工作流的智能自动化平台，试图把文献发现、实验设计、数据分析、论文撰写、同行评审与引用验证串成一条端到端的 Agent 流水线。

## 关键要点

- 采用“大脑、手脚、记忆”三层协同架构来组织科研自动化能力

- 支持 23 个阶段的一键式科研流程，并提供 Web 界面做实时进度跟踪

- 强调三层记忆、多模型路由，以及大规模可扩展的 Skill 生态

- 支持本地私有部署，并把引用验证、反 AI 痕迹检测、方法一致性检查纳入流程

## 关联概念

- [ClawHub](entities/ClawHub.md)

- [Harness Engineering](concepts/Harness Engineering.md)

## 来源引用

- [原文链接](https://mp.weixin.qq.com/s?__biz=MzIzNDE0Njk0Nw%3D%3D&mid=2247492713&idx=1&sn=697d58329dc78edaa065853085f14870&chksm=e9b942103b8b34109e2b7033053b11da342a126ea251fb119534dab8fadc86f8969f2a181ca7#rd)｜《科研智能自动化平台PaperClaw；大模型会话知识库llmwiki》｜源文章：科研智能自动化平台PaperClaw；大模型会话知识库llmwiki

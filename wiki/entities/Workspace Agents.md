---
title: Workspace Agents
type: entity
tags:
- Agent 协作模式
- AI 产品
- 多Agent协作
status: 审核中
confidence: high
last_compiled: '2026-04-28'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/f4edf19f42804c92b8787f0ff2bf6a04
notion_id: f4edf19f-4280-4c92-b878-7f0ff2bf6a04
---

## 定义

Workspace Agents 是 OpenAI 在 ChatGPT 中推出的组织级 Agent 产品形态，面向团队共享使用，能够跨工具执行复杂任务和长流程工作，并支持后台运行、定时触发与多成员协作。

## 关键要点

- 它是 GPTs 向组织级 Agent 平台演化后的代表形态，重点不再只是单次问答，而是持续执行任务。

- 底层由 Codex 驱动，因此兼具代码执行、工具调用与跨步骤处理复杂任务的能力。

- 它强调“共享”而非个人专属：一个 Agent 配置完成后，可以被整个团队复用，也能接入 Slack 等协作入口。

- 它把模板化业务流程沉淀为可运行的 Agent，例如产品反馈路由、销售跟进、周报生成、软件审批与第三方风控。

- 对企业落地而言，权限控制、审批、日志与合规可见性和模型能力同样重要。

## 来源引用

- [摘要：刚刚！OpenAI 凌晨甩出 Workspace Agents：一句话，给整个团队造一个 Agent](summaries/摘要：刚刚！OpenAI 凌晨甩出 Workspace Agents：一句话，给整个团队造一个 Agent.md)（[原文](https://mp.weixin.qq.com/s?__biz=MzU3NTg5MjU1Mw%3D%3D&mid=2247497089&idx=1&sn=4cc6bfa310b514c8e9d8b4367bf700c8&chksm=fc3fbce205cb5b3072e644178272bd0ccbe6c46ff678e7026f9c4847273899fff95151fd9e4f#rd)）

- [摘要：Your next 10 hires won't be human.](summaries/摘要：Your next 10 hires won't be human.md)（[原文](https://x.com/jiayuan_jy/status/2047014163097227478)）

- [摘要：OpenAI 刚发的 Workspace Agent，开源版来了](summaries/摘要：OpenAI 刚发的 Workspace Agent，开源版来了.md)（[原文](https://x.com/xiaohu/status/2047521587122106376)）

## 关联概念

- [Codex](entities/Codex.md)

- [持续任务自动化](concepts/持续任务自动化.md)

- [提示注入](concepts/提示注入.md)

- [Agent 治理](concepts/Agent 治理.md)

- [Compliance API](concepts/Compliance API.md)

- [Multica](entities/Multica.md)

- [Agent Runtime](concepts/Agent Runtime.md)

- [Agent Skills](concepts/Agent Skills.md)

- [Workspace-level Isolation](concepts/Workspace-level Isolation.md)

- [Coding Agent](concepts/Coding Agent.md)

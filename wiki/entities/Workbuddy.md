---
title: Workbuddy
type: entity
tags:
- 知识管理
- 工作流
status: 草稿
confidence: medium
last_compiled: '2026-04-23'
source_tags: ''
source_article_url: ''
notion_url: https://www.notion.so/Tizer/6638620a056943888b9bedf7735321c6
notion_id: 6638620a-0569-4388-8b9b-edf7735321c6
---

## 定义

Workbuddy 是一个把微信聊天入口接入个人知识库或工作流系统的工具，让用户可以直接通过微信发送文字、语音、照片等内容，再按预设规则写入 Obsidian 等目标系统。

## 关键要点

- 把聊天窗口变成轻量输入层，降低碎片信息进入知识库的门槛

- 适合随手记录灵感、语音备忘、照片素材等移动端场景

- 在以 Obsidian 为核心的工作流里，它承担“移动采集入口”的角色

- 价值不在单次记录，而在让采集流程可以持续、低摩擦地发生

## 来源引用

- [摘要：Obsidian + AI：8 个值得搭的自动化工作流，记得收藏！](summaries/摘要：Obsidian + AI：8 个值得搭的自动化工作流，记得收藏！.md)（[原文](https://x.com/xiangxiang103/status/2046845848244617468)）

- [摘要：实测Hy3 preview后我发现小看腾讯做大模型决心了](summaries/摘要：实测Hy3 preview后我发现小看腾讯做大模型决心了.md)（[原文](https://mp.weixin.qq.com/s?__biz=Mzg3MTk3NzYzNw%3D%3D&mid=2247506706&idx=1&sn=0ddf8c92a703a8eba003560eb9cc75b5&chksm=cf3b2d9a092249426bcaaeec74a3511426d134adae8ec03e81caae3124cbb7db3151e8dc023f#rd)）

## 关联概念

- [Obsidian](entities/Obsidian.md)
